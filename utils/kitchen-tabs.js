...
import { renderSubTabs } from '/my-hub/utils/sub-tabs.js';
...
export const KITCHEN_STORAGE_KEY = 'my-hub-kitchen-tab';
...
].filter(({ route }) => !window.myHub?.isModuleDisabled(route.slice(1)));

export function getLastKitchenRoute() {
  try {
    const stored = sessionStorage.getItem(KITCHEN_STORAGE_KEY);
    if (KITCHEN_ROUTES.includes(stored) && !window.myHub?.isModuleDisabled(stored.slice(1))) {
      return stored;
    }
  } catch { /* ignore */ }
  const first = ['meals', 'recipes', 'shopping', 'pantry'].find((m) => !window.myHub?.isModuleDisabled(m));
...
export function renderKitchenTabsBar(container, activeRoute) {
  ...
  renderSubTabs(container, {
    ...
    onChange: (route) => window.myHub?.navigate(route),
  });
}
