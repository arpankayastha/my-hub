 ...
 *
 * Side effects: writes a temporary database to /tmp/my-hub-screenshot.db
...
const DEMO_DB     = '/tmp/my-hub-screenshot.db';
const PORT        = 3099;
...
function initFlags(arg) {
  try {
    localStorage.setItem('my-hub-locale', arg.locale);
    localStorage.setItem('my-hub-onboarded', '1');
    localStorage.setItem('my-hub-install-dismissed', String(Date.now()));
    localStorage.setItem('my-hub-theme', arg.theme);
  } catch {}
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    document.querySelectorAll('.onboarding-overlay, my-hub-install-prompt').forEach((el) => el.remove());
  });
...
async function applyAppState(page, theme, locale) {
  await page.evaluate((a) => {
    localStorage.setItem('my-hub-locale', a.locale);
    localStorage.setItem('my-hub-onboarded', '1');
    localStorage.setItem('my-hub-install-dismissed', String(Date.now()));
    localStorage.setItem('my-hub-theme', a.theme);
