...
test('inspectCommand nutzt das passende Binary (podman auch bei podman-compose)', async () => {
  const pc = await detectEngine(checkFor(['podman --version', 'podman-compose --version']));
  assert.deepEqual(inspectCommand(pc, ['inspect', 'myhub']),
    { cmd: 'podman', args: ['inspect', 'myhub'] });

  const docker = await detectEngine(checkFor(['docker --version', 'docker compose version']));
  assert.deepEqual(inspectCommand(docker, ['inspect', 'myhub']),
    { cmd: 'docker', args: ['inspect', 'myhub'] });
});
...
test('podman-compose.yml trägt :Z-Labels, MY_HUB_HTTP_BIND und SESSION_SECURE-Default', () => {
  ...
  assert.match(src, /\$\{MY_HUB_HTTP_BIND:-0\.0\.0\.0\}/, 'kein konfigurierbares Host-Binding');
...
test('Quadlet-Unit existiert mit :Z-Volume und EnvironmentFile', () => {
  const src = readFileSync(new URL('../tools/quadlet/myhub.container', import.meta.url), 'utf8');
...
async function withServer(fn) {
  const prev = process.env.MY_HUB_INSTALLER_ROOT;
  process.env.MY_HUB_INSTALLER_ROOT = REPO_ROOT;
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MY_HUB_INSTALLER_ROOT;
    else process.env.MY_HUB_INSTALLER_ROOT = prev;
  }
}
