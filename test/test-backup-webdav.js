...
async function createTempBackup(name = 'my-hub-backup-2099-01-01T00-00-00-000Z.db') {
...
process.env.WEBDAV_BACKUP_PATH     = '/myhub/backups/';
...
describe('WebDAV Backup — service module', async () => {
  ...
  before(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'my-hub-webdav-test-'));
  ...
  it('getConfig() should read env vars', () => {
    ...
    assert.strictEqual(cfg.remotePath, '/myhub/backups/', 'remotePath');
  ...
  describe('uploadBackup()', async () => {
    ...
    it('should PUT the file to the remote server', async () => {
      ...
      const remotePath = `/myhub/backups/${path.basename(fp)}`;
      assert.ok(mockCtx.files.has(remotePath), 'file should exist on mock server');
    });

    it('should upload and list new my-hub-prefixed backups (post-rebrand naming)', async () => {
      const fp = await createTempBackup('my-hub-backup-2099-02-01T00-00-00-000Z.db');
      await webdav.uploadBackup(fp);
      assert.ok(
        mockCtx.files.has(`/myhub/backups/${path.basename(fp)}`),
        'my-hub-prefixed file should be uploaded'
      );
      const listed = await webdav.getRemoteFiles();
      assert.ok(
        listed.some((f) => f.filename.startsWith('my-hub-backup-')),
        'my-hub-prefixed file should be recognised by the file listing'
      );
    });
    ...
    it('should rotate when more than keep remote files exist', async () => {
      ...
      const names = [
        'my-hub-backup-2099-01-01T00-00-00-000Z.db',
        'my-hub-backup-2099-01-02T00-00-00-000Z.db',
        'my-hub-backup-2099-01-03T00-00-00-000Z.db',
        'my-hub-backup-2099-01-04T00-00-00-000Z.db',
      ];
      ...
      const remoteFiles = [...mockCtx.files.keys()].filter((k) =>
        k.startsWith('/myhub/backups/') && k.endsWith('.db')
      );
      assert.ok(
  ...
  describe('triggerUpload()', async () => {
    ...
    it('should throw when no local backup files exist', async () => {
      const emptyDir = await fs.mkdtemp(path.join(os.tmpdir(), 'my-hub-empty-'));
      try {
        await assert.rejects(
    ...
    it('should upload the most recent local backup', async () => {
      const fp = await createTempBackup('my-hub-backup-2099-06-01T12-00-00-000Z.db');
      const fileName = await webdav.triggerUpload(tmpDir);
      assert.ok(fileName.startsWith('my-hub-backup-'), 'returned file name');
      const remotePath = `/myhub/backups/${fileName}`;
