...
const REPRO_ICS = [
  'BEGIN:VCALENDAR',
  'VERSION:2.0',
  'PRODID:-//My Hub COUNT reproduction//EN',
  'BEGIN:VEVENT',
  ...
  'UID:my-hub-count-reproduction@example.invalid',
...
test('#513: COUNT+EXDATE import stays finite (10 instances, 9 visible, none after Dec 2)', async () => {
  ...
  const master = db.prepare(
    `SELECT * FROM calendar_events WHERE created_by = ? AND external_calendar_id = 'my-hub-count-reproduction@example.invalid'`,
  ).get(reproUid);
