 ...
 *        echte Datei im Repo-Wurzelverzeichnis: sie legt myhub.db an und
...
test('jede Suite, die server/db.js lädt, setzt DB_PATH', () => {
  ...
  assert.deepStrictEqual(
    ...
    + `myhub.db im Repo an. Setze DB_PATH=:memory: davor:\n  ${offenders.join('\n  ')}`
  );
});

test('die Reihenfolgeprüfung erkennt wirkungslose Zuweisungen', () => {
  const dir = mkdtempSync(join(tmpdir(), 'my-hub-dbguard-'));
