...
const TMP_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'my-hub-backup-routes-'));
...
process.env.DB_PATH = path.join(DB_DIR, 'my-hub.db');
...
const REMOTE_FILE = 'my-hub-backup-2026-01-01T00-00-00-000Z.db';
...
test('GET /database: lädt eine valide SQLite-Backup-Datei herunter', async () => {
  ...
  // Muss eine echte My Hub-DB sein (schema_migrations vorhanden) → für den späteren
