...
function tmpDir() {
  return mkdtempSync(join(tmpdir(), 'my-hub-encryption-'));
}
...
test('ohne DB_ENCRYPTION_KEY bleibt die Datenbank unverschlüsselt (Entwicklung)', async () => {
  const dbPath = join(tmpDir(), 'myhub.db');
...
test('eine unverschlüsselte Legacy-my-hub.db wird im selben Start umbenannt und verschlüsselt', async () => {
  // Der kritische Bestandsfall: Legacy-Dateiname UND gesetzter Key. Würde die
  // Rename-Migration der noch unverschlüsselten my-hub.db den Cipher-Key
  ...
  // kopie hieße my-hub.db.plaintext-backup — nicht der Name, den .env.example,
  // SECURITY.md und docs/installation.md zum Löschen nennen.
  const dir = tmpDir();
  const legacyPath = join(dir, 'my-hub.db');
  const dbPath = join(dir, 'myhub.db');
  seedPlaintextDb(legacyPath, 20);

  const mod = await bootDb(legacyPath, KEY);

  assert.ok(existsSync(dbPath), 'die Datenbank muss im selben Start nach myhub.db umgezogen sein');
...
test('ein vor der Umstellung erzeugtes Klartext-Backup bleibt einspielbar', async () => {
  ...
  const legacyPath = join(tmpDir(), 'myhub.db');
