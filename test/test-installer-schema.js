...
const TOTAL_KEYS = ORIGINAL_KEYS.length + GOOGLE_DRIVE_KEYS.length + 2 + P5_KEYS.length
  ...
  + WIZARD_EXTRA_KEYS.length; // + TZ + MY_HUB_HTTP_PORT

test('ENV_SCHEMA enthält alle Original-Keys, TZ, MY_HUB_HTTP_PORT, P5, Subscriptions und Dokument-WebDAV', () => {
  ...
  assert.ok(keys.includes('MY_HUB_HTTP_PORT'), 'Key fehlt: MY_HUB_HTTP_PORT');
...
test('Unraid deklariert die lokalen Dokumentspeicher-Werte advanced und optional', () => {
  const src = readFileSync(new URL('../templates/myhub.xml', import.meta.url), 'utf8');
  for (const key of DOCUMENT_STORAGE_LOCAL_KEYS) {
...
test('Jedes Container-Deployment schreibt Backups nach /backups (issue #579)', () => {
  ...
  const quadlet = readFileSync(new URL('../tools/quadlet/myhub.container', import.meta.url), 'utf8');
  assert.match(quadlet, /^Environment=BACKUP_DIR=\/backups$/m, 'Quadlet setzt BACKUP_DIR nicht');

  const unraid = readFileSync(new URL('../templates/myhub.xml', import.meta.url), 'utf8');
...
test('TZ und MY_HUB_HTTP_PORT haben writeToEnv: true', () => {
  for (const key of ['TZ', 'MY_HUB_HTTP_PORT']) {
    const entry = ENV_SCHEMA.find(e => e.key === key);
    assert.ok(entry, `${key} nicht in ENV_SCHEMA`);
    assert.equal(entry.writeToEnv, true, `${key}.writeToEnv ist nicht true`);
  }
...
test('install.html nimmt TZ und MY_HUB_HTTP_PORT ins gesendete env-Objekt auf', () => {
  const src = readFileSync(new URL('../tools/installer/install.html', import.meta.url), 'utf8');
  assert.match(src, /TZ:\s*S\.tz/, 'install.html sendet TZ nicht im env-Objekt');
  assert.match(src, /MY_HUB_HTTP_PORT:\s*S\.port/, 'install.html sendet MY_HUB_HTTP_PORT nicht im env-Objekt');
});
...
test('docker-compose.yml mappt den Host-Port über MY_HUB_HTTP_PORT mit Default 3000', () => {
  const src = readFileSync(new URL('../docker-compose.yml', import.meta.url), 'utf8');
  assert.match(
    src,
    /\$\{MY_HUB_HTTP_PORT:-3000\}:3000/,
    'Port-Mapping nutzt MY_HUB_HTTP_PORT nicht mit Default :-3000 (Container-Port muss 3000 bleiben)'
  );
  assert.doesNotMatch(
    src,
...
test('install.sh schreibt TZ und MY_HUB_HTTP_PORT in die generierte .env', () => {
  const src = readFileSync(new URL('../install.sh', import.meta.url), 'utf8');
  assert.match(src, /^TZ=\$\{MY_HUB_TZ\}/m, 'install.sh schreibt TZ=${MY_HUB_TZ} nicht in den .env-Block');
  assert.match(src, /^MY_HUB_HTTP_PORT=\$\{MY_HUB_PORT\}/m, 'install.sh schreibt MY_HUB_HTTP_PORT=${MY_HUB_PORT} nicht in den .env-Block');
});

test('.env.example dokumentiert MY_HUB_HTTP_PORT', () => {
  const src = readFileSync(new URL('../.env.example', import.meta.url), 'utf8');
  assert.match(src, /MY_HUB_HTTP_PORT/, '.env.example dokumentiert MY_HUB_HTTP_PORT nicht');
});
