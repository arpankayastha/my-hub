...
test('date inputs: accept slash, dot, and hyphen separators for DMY dates', () => {
  localStorage.setItem('my-hub-date-format', 'dmy');
...
test('date inputs: accept hyphen separators for YMD dates', () => {
  localStorage.setItem('my-hub-date-format', 'ymd');
  assert.equal(parseDateInput('2026-5-6'), '2026-05-06');
  assert.equal(parseDateInput('2026/05/06'), '2026-05-06');
  assert.equal(parseDateInput('2026.05.06'), '2026-05-06');
});

test('task + recurrence date fields use the shared my-hub-datepicker', () => {
  ...
  assert.match(tasksSource, /<my-hub-datepicker type="date"[\s\S]*?name="start_date"/);
  assert.match(tasksSource, /<my-hub-datepicker type="date"[\s\S]*?name="due_date"/);
  assert.match(tasksSource, /<my-hub-datepicker type="time"[\s\S]*?name="due_time"/);
  assert.match(rruleSource, /<my-hub-datepicker type="date"[\s\S]*?id="\$\{prefix\}-rrule-until"/);
...
test('scheduleUndoableDelete: Undo verhindert den Server-Delete', async () => {
  ...
  global.window.myHub = { showToast: (_msg, _type, _duration, undoFn) => { capturedUndo = undoFn; } };
...
test('parseTimeInput: bare hour (24 h) expands to HH:00', () => {
  localStorage.setItem('my-hub-time-format', '24h');
...
test('formatTimeInput: bare hour (12 h) formats with AM/PM', () => {
  localStorage.setItem('my-hub-time-format', '12h');
...
test('parseDateInput: 8 raw digits (MDY)', () => {
  localStorage.setItem('my-hub-date-format', 'mdy');
