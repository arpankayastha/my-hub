...
test('Definiert das Custom Element my-hub-datepicker', () => {
  assert(/customElements\.define\(\s*'my-hub-datepicker'/.test(comp), 'Tag-Name muss my-hub-datepicker sein');
});
test('Registrierung ist idempotent (guard gegen Doppel-Define)', () => {
  assert(/if\s*\(\s*!customElements\.get\(\s*'my-hub-datepicker'\s*\)\s*\)/.test(comp), 'Define muss geguardet sein');
});
