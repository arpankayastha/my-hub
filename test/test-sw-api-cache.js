 ...
 *   - activate entfernt alte my-hub-api-*-Caches der Vorversionen
...
async function apiCacheName(env) {
  return (await env.caches.keys()).find((n) => n.startsWith('my-hub-api-'));
}
...
test('activate entfernt alte Vorversions- und Legacy-my-hub-Caches, behält aktuelle Versions-Caches', async () => {
  const env = loadSw();
  // Vorzustand: alter API-Cache, ein Legacy-`my-hub-*`-Cache aus der Zeit vor dem
  // My Hub-Rename + aktueller Shell- und API-Cache der laufenden Version.
  const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
  await env.caches.open('my-hub-api-0.0.1');                 // Vorversion → löschen
  await env.caches.open('my-hub-shell-0.0.1');               // Legacy-Rename → löschen
  await env.caches.open(`my-hub-shell-${pkg.version}`);     // aktuell → behalten
  await env.caches.open(`my-hub-api-${pkg.version}`);       // aktuell → behalten

  await dispatchActivate(env);

  assert.equal(await env.caches.has('my-hub-api-0.0.1'), false, 'alter API-Cache muss weg sein');
  assert.equal(await env.caches.has('my-hub-shell-0.0.1'), false, 'Legacy-my-hub-Cache muss weg sein');
  assert.equal(await env.caches.has(`my-hub-shell-${pkg.version}`), true, 'aktueller Shell-Cache bleibt');
  assert.equal(await env.caches.has(`my-hub-api-${pkg.version}`), true, 'aktueller API-Cache bleibt');
});
