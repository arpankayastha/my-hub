...
npm run test:db
npm run test:rename-migration   # My Hub→My Hub Identifier-Migration: seamless rename invariants
...
npm run test:db-isolation       # Test-Isolation: keine Suite lädt server/db.js ohne wirksames DB_PATH (init() beim Import würde sonst eine echte myhub.db im Repo-Root anlegen); prüft auch die Reihenfolge, da eine Zuweisung nach einem statischen Import zu spät kommt
npm run test:tasks
...
npm run test:category-manager   # generic my-hub-category-manager component + budget wiring
...
npm run test:datepicker         # my-hub-datepicker: ISO-Wertkontrakt, form-association, Popover/Touch, min/max, i18n-Vollständigkeit
...
npm run test:google-outbound   # Löschen + Ändern + Umziehen My Hub → Google (#593): Tombstones, Dirty-Marker, events.move, 404/410, Retry-Limit, Inbound-Konfliktschutz; dazu Serien als Master (EXDATE aus Absagen/Verschiebungen, Altbestand-Zusammenführung nur beim Full-Resync)
...
npm run test:caldav-outbound   # Löschen + Ändern + Umziehen My Hub → CalDAV/Apple (#593): ICS-Patcher (Teilnehmer/Alarme/Overrides bleiben), Objekt-URL-Auflösung, Umzug = create+delete, Sofortversuch ohne Kalenderabruf
...
npm run test:google-drive-storage   # Google Drive als Dokument-Ablage: eigenes Credential-Paar (fail-closed bei halber Konfiguration), OAuth-Callback legt My Hub/Documents an und wählt nie Drive als Kalender
