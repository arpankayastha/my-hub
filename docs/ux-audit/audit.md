# UX/UI-Audit My Hub - Phase 1 (read-only)
...
Empfehlung: Shopping auf `my-hub-category-manager` migrieren (API deckt Rename-Kaskade/Delete-Fallback ggf. noch nicht ab - vorher prüfen, Tests `test:shopping-routes` decken die Kaskaden).
Severity: P3 | Aufwand: M
...
| Toast | `window.myHub.showToast` types default/success/danger/warning (`router.js:2577`) | app-weit | - |
