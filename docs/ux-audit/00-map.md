...
| Component-Library | Keine externe. Eigene Web Components (`my-hub-`/`my-hub-`-Prefix) + CSS-Klassen-Patterns in `layout.css` | `public/components/*` |
...
| `datepicker.js` | `my-hub-datepicker`, ISO-Kontrakt, Popover primär | eigener Test |
...
| `user-multi-select.js`, `my-hub-install-prompt.js`, `my-hub-locale-picker.js` | Mehrfach-Zuweisung, PWA-Prompt, Sprachwahl | - |
...
- Dev-Server: `preview_start` (launch.json `myhub`, Port 3000, Temp-DB `${TMPDIR}/my-hub-preview.db`).
