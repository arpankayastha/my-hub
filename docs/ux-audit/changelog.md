...
| F-15 | `16b4d3fd` | Einkauf nutzt den geteilten `my-hub-category-manager`: `_keyOf`-Helper (String-`key` oder numerische `id`), Icon-Rendering in Zeilen; 221-Zeilen-Duplikat `shopping-category-manager.js` entfernt (inkl. SW-Precache, `.shopping-cat-*`-Styles, Typo-Rollen); Test-Verträge in test-shopping.js/test-frontend-audit.js neu gepinnt; Deep-Link `?manage=categories` live verifiziert. |
