...
5.  Philosophie          [VORGEZOGEN] Umbenannt "Why My Hub", neue Reihenfolge
...
- Optisch identisch
- GitHub-Button: Live-Sternzahl per `fetch('https://api.github.com/repos/arpankayastha/my-hub')`, gecacht in `sessionStorage` als `gh-stars`, Fallback "GitHub" wenn Fetch scheitert oder > 3 s dauert
...
- **Section Label:** `"Philosophy"` → `"Why My Hub"` / `"Warum My Hub"`
...
| One command launches a setup wizard in your browser. No configuration by hand. | Create an admin account and choose your preferences. Takes 2 minutes. | Add family members, set roles, and start using My Hub together. |
...
**Subtext:** `"My Hub is free, open-source, and built for families who value their privacy."` → `"Free. Open source. Runs on your server. No subscriptions, ever."` / `"Kostenlos. Open Source. Läuft auf eurem Server. Keine Abos, nie."`
...
| `why_label` | Why My Hub | Warum My Hub |
...
| `setup_3_d` | Add family members, set roles, start using My Hub. | Familienmitglieder hinzufügen und loslegen. |
...
- **GitHub API Fetch:** Einmalig beim Laden, `sessionStorage`-Key `my-hub-gh-stars`, Timeout 3 s, Fallback auf leerem String (Button zeigt dann nur "GitHub" ohne Zahl). Kein separater API-Call für Nav und Social Proof Bar — beide lesen aus demselben Cache.
