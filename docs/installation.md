...
Three ways to get My Hub running from scratch:

### Option A — Web Installer (recommended, all platforms)

```bash
git clone https://github.com/arpankayastha/my-hub.git && cd myhub
...
```bash
curl -O https://raw.githubusercontent.com/arpankayastha/my-hub/main/docker-compose.yml
curl -O https://raw.githubusercontent.com/arpankayastha/my-hub/main/.env.example
...
```bash
curl -O https://raw.githubusercontent.com/arpankayastha/my-hub/main/podman-compose.yml
curl -O https://raw.githubusercontent.com/arpankayastha/my-hub/main/.env.example
cp .env.example .env  # set SESSION_SECRET and DB_ENCRYPTION_KEY
...
`docker compose exec myhub node setup.js` (or the matching `podman compose … exec`).

---

# Installation Guide

Complete setup instructions for My Hub - from Docker installation to your first login.
...
For local-only access, the Docker container is all you need. If you want to access My Hub from other devices on your network or the internet, add Nginx as a reverse proxy with SSL.
...
Docker. My Hub supports Podman out of the box: both installers auto-detect it, and a
...
There are six ways to get My Hub running. **Option A** (web installer) is recommended for most users — it walks you through every step in your browser. **Option B** (pre-built image) is a quick manual alternative. **Option C** (build from source) is for contributors or custom builds. **Options D–F** install directly from a NAS/home-server app store with no terminal required: **Option D** (TrueNAS SCALE), **Option E** (Umbrel), and **Option F** (Unraid).
...
```bash
git clone https://github.com/arpankayastha/my-hub.git
cd myhub
```
...
- Basics — timezone (`TZ`) and HTTP host port (`MY_HUB_HTTP_PORT`)
...
Docker pulls `ghcr.io/arpankayastha/my-hub:latest` automatically. No build step, no Node.js installation needed.
...
```
myhub  | [My Hub] Server läuft auf Port 3000
myhub  | [My Hub] Umgebung: production
myhub  | [Sync] Auto-Sync alle 15 Minuten aktiv.
```
...
On the first visit, My Hub detects that no account exists yet and guides you through
...
After you submit, My Hub creates the admin, signs you in automatically, and the setup
...
```bash
docker compose exec myhub node setup.js
```
...
No terminal required. My Hub is available directly in the TrueNAS SCALE Community Apps Catalog.

#### 1. Open the Apps Catalog

In your TrueNAS SCALE web UI, go to **Apps → Discover Apps** and search for **My Hub**.
...
No terminal required. My Hub is available in the Umbrel App Store — everything runs on, and stays on, your Umbrel.

#### 1. Open the App Store

In your Umbrel dashboard, open the **App Store** and search for **My Hub**.
...
Launch My Hub from your Umbrel home screen. The first visit guides you through creating your admin account in the browser.
...
No terminal required. My Hub ships as an Unraid Community Applications template.

#### 1. Open Community Applications

In Unraid, open the **Apps** tab (the Community Applications plugin) and search for **My Hub**.
...
Click **Apply**. Once the container is running, click the My Hub icon → **WebUI**. The first visit guides you through creating your admin account in the browser.
...
| `MY_HUB_HTTP_PORT` | Host port that the compose file maps to the container's port 3000. Change this to expose My Hub on a different host port; the app inside the container always listens on 3000. | `3000` | No |
| `MY_HUB_HTTP_BIND` | Host bind address for the published port (`podman-compose.yml` only). Set to `127.0.0.1` for rootless Podman behind a reverse proxy on the same host. | `0.0.0.0` | No |
...
are configured in the UI and do not require environment variables. The My Hub backend container or
...
- **The app must be installed to the Home Screen.** iOS delivers Web Push only to installed
  home-screen web apps, never to a Safari tab. Open My Hub in Safari, then Share ->
  "Add to Home Screen".
...
- **Check iOS Settings -> Notifications -> My Hub**: "Allow Notifications" must be on, and a Focus
...
| `EMAIL_FROM_NAME` | Sender display name. | `My Hub` | No |
| `BASE_URL` | Absolute origin used to build password-reset links and calendar export-feed URLs, e.g. `https://myhub.example.com`. **Required for reset emails to be sent** — the request `Host` header is never trusted as a fallback, to prevent reset-link poisoning. The export feed falls back to the request's protocol/host when unset. | - | No* |

\* Not required to start My Hub. Without it (or without SMTP configured) the self-service reset
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `DB_PATH` | Path to the SQLite database file inside the container | `/data/myhub.db` | No |
...
Uploads fail closed: if WebDAV cannot accept the file, My Hub rejects the upload instead of silently
...
below. My Hub requests only `https://www.googleapis.com/auth/drive.file`; it cannot browse arbitrary
...
activate it. New files are placed in the visible private `My Hub/Documents` folder; the opaque Drive
...
> **Access and backup boundary:** My Hub visibility settings only control access through My Hub.
...
events created, edited, deleted, or moved to another calendar in My Hub are applied in Google as
...
My Hub from changing anything in Google while still importing normally.
...
My Hub are applied on the server as well, and changes made there flow back. An outbound change is
attempted right when you save and retried by the next sync run if the server cannot be reached.
Editing preserves everything the server holds that My Hub does not — attendees, alarms, categories
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `OIDC_ISSUER` | OIDC provider issuer URL (e.g. `https://authentik.example.com/application/o/myhub/`) | - | No |
...
| `OIDC_REDIRECT_URI` | OAuth callback URL — must be registered with the provider (e.g. `https://myhub.example.com/api/v1/auth/oidc/callback`) | - | No |
...
| `WEBDAV_BACKUP_PATH` | Remote directory path for backup files | `/myhub/backups/` | No |
...
> **Optional for local access, required for network/internet access.** If you only access My Hub on the same machine (localhost), you can skip this section.

When exposing My Hub to your local network or the internet, you need HTTPS for security. Nginx acts as a reverse proxy that handles SSL termination and forwards requests to the Docker container.
...
My Hub ships with an example configuration. Copy it to Nginx:

```bash
sudo cp nginx.conf.example /etc/nginx/sites-available/myhub
...
On RHEL-based systems you can run My Hub as a rootless systemd service via Podman
[Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html). My Hub
ships a ready-made unit at `tools/quadlet/myhub.container`.

```bash
# 1. Create the data folders and drop your generated .env in place
mkdir -p ~/.local/share/myhub/{data,backups,modules} ~/.config/myhub
cp /path/to/myhub/.env ~/.config/myhub/.env

# 2. Install the Quadlet unit
mkdir -p ~/.config/containers/systemd
cp tools/quadlet/myhub.container ~/.config/containers/systemd/

# 3. Generate and start the service
systemctl --user daemon-reload
systemctl --user start myhub
...
> **One-time step when upgrading past the container rename (My Hub → My Hub):** the Docker/Podman
> service and container were renamed from `myhub` to `myhub`. Your data is safe - the database
> volume is unchanged and the app migrates an existing `my-hub.db` to `myhub.db` automatically on
> first start. But the old `myhub` container lingers as an orphan and keeps holding host port 3000,
> which prevents the new `myhub` container from starting. Run the update once with
> `--remove-orphans`:
>
> ```bash
...
> ```
>
> TrueNAS, Unraid and Podman-Quadlet installs keep the legacy `myhub` slug and are unaffected.
...
The SQLite database lives in the host folder configured through `DATA_DIR` and is mounted at `/data` inside the container. The database file is `/data/myhub.db`.
...
```bash
docker compose exec myhub node -e "import('./server/db.js').then(async db => { await db.backupToFile('/data/my-hub-backup.db'); process.exit(0); })"
docker cp myhub:/data/my-hub-backup.db ./my-hub-backup-$(date +%Y%m%d).db
```
...
```bash
SERVICE=myhub
BACKUP="$PWD/my-hub-backup-20260401.db"
docker compose stop "$SERVICE"
docker compose run --rm -v "$BACKUP:/tmp/my-hub-restore.db:ro" --entrypoint sh "$SERVICE" -c 'set -eu; target="${DB_PATH:-/data/myhub.db}"; case "$target" in */my-hub.db) target="${target%/my-hub.db}/myhub.db";; esac; stamp=$(date -u +%Y%m%dT%H%M%SZ); if [ -f "$target" ]; then cp "$target" "$target.pre-restore-$stamp"; fi; rm -f "$target-wal" "$target-shm"; cp /tmp/my-hub-restore.db "$target"; chown node:node "$target" 2>/dev/null || true'
...
```bash
DB_PATH=/path/to/myhub.db node --import dotenv/config scripts/restore-backup.js ./my-hub-backup-20260401.db
```

The restore helper validates that the file is an My Hub database before replacing the active database. It also keeps a pre-restore copy next to the database file for emergency rollback.
...
```
0 3 * * * docker compose exec -T myhub node -e "import('./server/db.js').then(async db => { await db.backupToFile('/data/my-hub-cron-backup.db'); process.exit(0); })" && docker cp myhub:/data/my-hub-cron-backup.db /path/to/backups/my-hub-$(date +\%Y\%m\%d).db
```
...
Either stop the conflicting process, or change the host port in your `.env` file — `docker-compose.yml` maps `MY_HUB_HTTP_PORT` to the container's port 3000 automatically:

```bash
MY_HUB_HTTP_PORT=8080
```
