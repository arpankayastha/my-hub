# Datenschutz-Hinweise für Selfhoster (My Hub)
...
> Dieses Dokument richtet sich an **Betreiber, die My Hub in einer Umgebung
> einsetzen, die unter die DSGVO fällt** — also typischerweise an einen
> Wohnsitz, ein Unternehmen oder eine Organisation in der EU/EWR. Wenn du My Hub
...
2. [Externe Dienste, die My Hub kontaktiert](#2-externe-dienste-die-my-hub-kontaktiert)
...
Betreiber** der My Hub-Instanz Verantwortlicher i. S. v. Art. 4 Nr. 7 DSGVO. Das
bedeutet u. a.:
...
## 2. Externe Dienste, die My Hub kontaktiert
...
- **Was wird übertragen:** Geo-Koordinaten oder Ortsname (je nach
  Benutzer-Einstellung) sowie die IP-Adresse deines My Hub-Servers (nicht die
...
- **Wer ist Empfänger?** Der **vom Nutzer konfigurierte** CalDAV-/CardDAV-
  ...
  My Hub selbst leitet nichts weiter.
...
- **Empfehlung:** Trage die konkret eingerichteten Sync-Endpoints in dein
  Verarbeitungsverzeichnis (Abschnitt 5) ein — My Hub kennt sie nicht zentral,
...
- **Aktiv nur, wenn:** du WebDAV-Backups in deinen Backup-Einstellungen
  konfigurierst.
- **Was wird übertragen:** Backup-Archive deiner My Hub-Instanz mit allen
...
- **Empfehlung:** Verschlüssele Backups **vor** der Übertragung (My Hub bietet
...
- **Was wird übertragen:** neu hochgeladene Dokumentdateien einschließlich
  ...
  des My Hub-Servers. Dateinamen werden nicht als Objektpfad übernommen; die
...
- **Was wird übertragen:** neue Dokumentdateien und Kalenderanhänge, generierte
  Dateinamen, Server-IP sowie OAuth-Zugriffs-/Refresh-Token. My Hub liest zusätzlich
...
- **Zugriffsgrenze:** My Hubs Sichtbarkeitseinstellungen steuern nur den Zugriff
  über My Hub. Alle Personen mit Zugriff auf den verbundenen Google-Drive-Ordner
  `My Hub/Documents` können sämtliche dort gespeicherten Dateien sehen. Teile diesen Ordner nicht unnötig.
- **Löschung und Aufbewahrung:** Das Löschen eines Drive-Dokuments in My Hub löscht
...
- **Was ist das?** My Hub stellt einen **MCP-Endpoint** bereit, über den ein
  ...
  **provider-neutral** - er funktioniert mit einem **lokal gehosteten LLM**
  ...
  (z. B. Claude Desktop). My Hub selbst ruft **keinen** KI-Anbieter auf; der
...
My Hub verwendet einen **eigenen, dependency-freien Logger**
...
Wenn du My Hub **nur für dich selbst** oder **mit Familienmitgliedern unter
...
- Du nutzt My Hub für **berufliche/geschäftliche Zwecke** (z. B. Steuerberater,
