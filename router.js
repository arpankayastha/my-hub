...
import { api, auth } from '/my-hub/api.js';
import { canAccessNavModule, navModuleAccess } from '/my-hub/permissions.js';
import { clearApiCache } from '/my-hub/sw-register.js';
import { initI18n, getLocale, t, formatDate, formatTime } from '/my-hub/i18n.js';
import { esc } from '/my-hub/utils/html.js';
import { wireScrollFade } from '/my-hub/utils/ux.js';
import { createSidebarLogomark } from '/my-hub/utils/logomark.js';
import { init as initReminders, stop as stopReminders } from '/my-hub/reminders.js';
import { initPush, stopPush } from '/my-hub/push.js';
import { numberLocaleFor } from '/my-hub/settings/region-presets.js';
import { isKitchenRoute, isKitchenModule, getLastKitchenRoute } from '/my-hub/utils/kitchen-tabs.js';
import { getLastHealthRoute, HEALTH_ROUTES } from '/my-hub/utils/health-tabs.js';
import { activityType } from '/my-hub/utils/health-activity.js';
import { buildHelpRows } from '/my-hub/utils/help.js';
import { renderSkeletonList } from '/my-hub/utils/skeleton.js';
import { openModal, confirmModal } from '/my-hub/components/modal.js';
import '/my-hub/components/datepicker.js';
import { NAV_ICONS } from '/my-hub/nav-icons.js';
import { RENAMED_SETTINGS_SOURCE_PATHS, SETTINGS_LEAVES } from '/my-hub/settings/registry.js';
import { toAppUrl, fromAppUrl, assetUrl, importModule } from '/my-hub/app-path.js';
import {
  NAV_SECTION,
  resolveMobileNavOrder,
  sortNavigationItems,
} from '/my-hub/settings/module-order.js';
import {
...
const APP_VERSION_STORAGE_KEY = 'my-hub-app-version';
...
async function navigate(path, userOrPushState = true, pushState = true) {
  ...
  try {
    ...
    if (pushState) {
      history.pushState({ path: basePath }, '', toAppUrl(path));
    }
...
async function syncPreferencesOnce() {
  ...
  try {
    ...
    if (dateFormat) {
      localStorage.setItem('my-hub-date-format', dateFormat);
    }
    const timeFormat = res?.data?.time_format;
    if (timeFormat) {
      localStorage.setItem('my-hub-time-format', timeFormat);
    }
    ...
    if (numberLocale) {
      localStorage.setItem('my-hub-number-locale', numberLocale);
    } else {
      localStorage.removeItem('my-hub-number-locale');
    }
...
const FAB_SEEN_KEY = (module) => `myhub:fabSeen:${module}`;
const FAB_SEEN_MAX = 5;
const SIDEBAR_COLLAPSED_KEY = 'myhub.sidebar.collapsed';
 ...
 */
const TOAST_SUCCESS_KEY = 'myhub:toastSuccessCount';
...
window.addEventListener('error', (e) => {
  ...
  console.error('[My Hub] Unbehandelter Fehler:', e.error ?? e.message);
...
window.addEventListener('unhandledrejection', (e) => {
  ...
  console.error('[My Hub] Unbehandeltes Promise-Rejection:', e.reason);
...
window.addEventListener('popstate', (e) => {
  navigate(e.state?.path || fromAppUrl(location.pathname), false);
});
...
(async () => {
  try {
    // Vorab-Theme-Anwendung ohne Abhängigkeit von window.myHub
    const stored = localStorage.getItem('my-hub-theme');
    if (stored === 'dark') {
    ...
    navigate(fromAppUrl(location.pathname), false);
  } catch (err) {
...
window.myHub = {
  ...
  isModuleDisabled,
  applyTheme: (value) => {
    localStorage.setItem('my-hub-theme', value);
    if (value === 'dark') {
      document.documentElement.setAttribute('data-theme', 'dark');
    } else if (value === 'light') {
      document.documentElement.setAttribute('data-theme', 'light');
...
// globale API `window.myHub` geschrieben. Ohne diesen Alias würfen ihre Aufrufe
// (window.myHub.navigate/showToast …) nach dem Rename, und der Router würde das
// Modul als fehlerhaft deaktivieren. Der Alias hält den Upgrade-Pfad nahtlos.
window.myHub = window.myHub;
