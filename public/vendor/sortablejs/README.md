...
- **Plugins included:** upstream mounts `AutoScrollPlugin` and the remove/revert-on-spill plugins by default inside `sortable.esm.js` - no extra plugin files needed for My Hub's use (single-column reorderable lists).
