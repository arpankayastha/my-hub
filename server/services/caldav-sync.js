...
function buildCalDAVICS(event) {
  const uid  = `my-hub-${event.id}@myhub.local`;
  const now  = new Date().toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//My Hub//CalDAV Sync//EN',
    'BEGIN:VEVENT',
    `UID:${uid}`,
    `DTSTAMP:${now}`,
...
async function sync({ createClient } = {}) {
  ...
  for (const account of accounts) {
    try {
      ...
      // Outbound sync: My Hub → CalDAV (events with target_caldav_account_id)
