...
export function createEmailService({ db, nodemailer = nodemailerDefault, env = process.env } = {}) {
  ...
  function getRawConfig() {
    ...
    return {
      ...
      fromName: resolve('fromName') || 'My Hub',
    };
  }
  ...
  async function sendTest(to) {
    try {
      ...
      await transport.sendMail({
        from: fromHeader(c),
        to,
        subject: 'My Hub SMTP test',
        text: 'This is a test message confirming your My Hub SMTP configuration works.',
        html: '<p>This is a test message confirming your My Hub SMTP configuration works.</p>',
      });
      return { ok: true };
    } catch (err) {
