...
async function sync() {
  ...
  for (const calendarId of calendarIds) {
    ...
    do {
      ...
      // immer liefern, und wie My Hub Serien lokal führt und expandiert (#593).
...
// Subtract 1 day to convert to My Hub-style inclusive end date.
...
// My Hub stores inclusive end dates. Add 1 day when sending to Google (exclusive).
...
// My Hub speichert getimte Events als "YYYY-MM-DDTHH:MM" (ohne Sekunden,
...
function localEventToGoogle(event, colorMap = {}, timeZone = serverTimeZone()) {
  ...
  if (allDay) {
    ...
    // My Hub speichert getimte Events als naive Wanduhrzeit ohne Zone. Ohne
    ...
    // steht in Google dieselbe Uhrzeit wie in My Hub.
