...
// New backups use the `my-hub-` prefix; pre-rebrand files use `my-hub-`.
...
const BACKUP_FILE_PREFIX = 'my-hub-backup-';
const LEGACY_FILE_PREFIX = 'my-hub-backup-';
...
export function getConfig() {
  ...
  const rawPath  = ENV_PATH ?? cfgGet('webdav_backup_path') ?? '/myhub/backups/';
...
export function saveConfig(data) {
  ...
  if (data.remotePath !== undefined) {
    const p = String(data.remotePath).trim() || '/myhub/backups/';
