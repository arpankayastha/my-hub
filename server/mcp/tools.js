...
const OPENAPI_TOOLS = [
  {
    ...
    description: 'List My Hub REST API operations reachable through call_api_operation.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Return OpenAPI metadata for one My Hub API operation key.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Call any My Hub REST API operation from the live OpenAPI spec. Runs with the permissions of the authenticated MCP token — admin-only routes require an admin token.',
    inputSchema: {
      type: 'object',
      properties: {
        operation_key: { type: 'string', description: 'Operation key returned by list_api_operations.' },
