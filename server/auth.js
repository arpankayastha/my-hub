...
// Session-Cookie-Name. Legacy „My Hub"-Installationen nutzten `myhub.sid`; der
// Name ist nun `myhub.sid`. Der Wechsel ist NAHTLOS (kein Zwangs-Logout): der
// signierte Session-Wert ist nur über den Wert (die sid) signiert, nicht über den
// Cookie-Namen — daher kann ein vorhandenes `myhub.sid` transparent als
// `myhub.sid` weitergereicht werden (siehe sessionMiddleware unten).
const SESSION_COOKIE = 'myhub.sid';
const LEGACY_SESSION_COOKIE = 'myhub.sid';
 ...
 * Trägt ein vorhandenes `myhub.sid`-Cookie einmalig als `myhub.sid` nach, sodass
...
function sessionMiddleware(req, res, next) {
  ...
  if (header && header.includes(`${LEGACY_SESSION_COOKIE}=`) && !header.includes(`${SESSION_COOKIE}=`)) {
    const match = header.match(/(?:^|;\s*)myhub\.sid=([^;]+)/);
    if (match) {
      ...
      //    nach dem Verwerfen von myhub.sid komplett ohne Session-Cookie zurück.
