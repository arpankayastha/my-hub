...
// --------------------------------------------------------
// Pfad-Auflösung (Legacy-Migration my-hub.db → myhub.db)
// --------------------------------------------------------
//
// My Hub hieß früher „My Hub". Die DB-Datei lag standardmäßig unter `my-hub.db`
// (bzw. `/data/my-hub.db` in allen ausgelieferten Docker-Templates). Damit
// Bestands-Nutzer beim Update NICHTS von Hand ändern müssen, leiten wir den
// effektiven Pfad ab:
//   - kein DB_PATH gesetzt              → <root>/myhub.db
//   - DB_PATH endet auf my-hub.db        → <dir>/myhub.db  (Legacy-Default erkannt)
//   - DB_PATH endet auf myhub.db       → <dir>/myhub.db  (neuer Default)
//   - beliebiger anderer DB_PATH        → unverändert (Custom-Pfad wird respektiert)
// In allen „managed" Fällen wird eine vorhandene `my-hub.db` beim Start einmalig
// nach `myhub.db` migriert. Dass auch der NEUE Default `myhub.db` als managed
// gilt, ist bewusst: aktualisiert ein Bestands-User seine Compose-Datei auf den
// neuen Default, läge die Daten weiter in `my-hub.db` — die Migration greift trotzdem.
let DB_PATH;
let LEGACY_DB_PATH; // null außer bei „managed" Layout
{
  ...
  const isManagedLayout = !configured || base === 'my-hub.db' || base === 'myhub.db';
  DB_PATH = isManagedLayout ? path.join(baseDir, 'myhub.db') : configured;
  LEGACY_DB_PATH = isManagedLayout ? path.join(baseDir, 'my-hub.db') : null;
}

/**
 * Einmalige, idempotente Migration der Legacy-Datenbankdatei `my-hub.db` → `myhub.db`.
...
function migrateLegacyDbFile() {
  ...
  //    TRUNCATE-Checkpoint nachweislich gelang, trägt `my-hub.db` allein den
  ...
  if (!checkpointed) {
    ...
    // (SQLite öffnet my-hub.db samt intaktem WAL); nächster Boot versucht es erneut.
  ...
  //    legt -wal/-shm für myhub.db bei Bedarf neu an).
  try {
...
function init({ plaintextBackup = true } = {}) {
  ...
  if (!path.isAbsolute(DB_PATH)) {
    log.warn(
      ...
      `Data will be lost on container restart. Use an absolute path, e.g. DB_PATH=/data/myhub.db`
    );
  }
...
const MIGRATIONS = [
  ...
  },
  {
    ...
    up: `
      ...
      -- Farbänderungen blieben damit in My Hub hängen.
      --
...
function validateBackupFile(sourcePath) {
  ...
  try {
    ...
    `).get();
    if (!row) {
      throw new Error('Backup file is not a valid My Hub database.');
    }
