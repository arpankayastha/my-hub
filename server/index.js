...
const logMy Hub = createLogger('My Hub');
...
app.get('/feed/calendar/:token.ics', feedLimiter, (req, res) => {
  try {
    ...
    res.set('Content-Disposition', 'inline; filename="myhub.ics"');
...
// --------------------------------------------------------
app.listen(PORT, () => {
  logMy Hub.info(`Server running on port ${PORT} | Version ${APP_VERSION}`);
  logMy Hub.info(`Environment: ${process.env.NODE_ENV || 'development'}`);
