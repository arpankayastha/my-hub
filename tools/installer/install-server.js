#!/usr/bin/env node
/**
 * My Hub Web Installer — temporary setup server.
...
function projectRoot() {
  return process.env.MY_HUB_INSTALLER_ROOT
    ? resolve(process.env.MY_HUB_INSTALLER_ROOT)
    : DEFAULT_ROOT;
}
...
export function renderEnvFile(env) {
  const lines = ['# Generated by My Hub Installer'];
...
async function route(req, res, server) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/preflight') {
    try {
      ...
      const { cmd, args } = inspectCommand(engine, ['inspect', '--format', '{{.State.Status}}', 'myhub']);
  ...
  if (req.method === 'GET' && url.pathname === '/api/status') {
    const engine = await getEngine();
    const health = inspectCommand(engine, ['inspect', '--format', '{{.State.Health.Status}}', 'myhub']);
    const stateCmd = inspectCommand(engine, ['inspect', '--format', '{{.State.Status}}', 'myhub']);
  ...
  if (req.method === 'POST' && url.pathname === '/api/create-admin') {
    try {
      ...
      const myhubPort = Number.isInteger(body.port) && body.port > 0 && body.port < 65536
        ? body.port : 3000;
      ...
      const result = await new Promise((resolve, reject) => {
        const opts = {
          hostname: 'localhost', port: myhubPort,
        ...
        const proxyReq = http.request(opts, proxyRes => {
          ...
          proxyRes.on('end', () => {
            ...
            catch { resolve({ status: proxyRes.statusCode, body: { error: 'Invalid response from My Hub' } }); }
          });
        });
