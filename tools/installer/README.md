# My Hub Web Installer

A browser-based setup wizard for My Hub. Run it once to configure your `.env`,
...
1. Detects the container engine (Docker or Podman), checks its prerequisites, and
   reports any existing `.env` file or running `myhub` container before you start
2. Lets you pick a setup path on the welcome screen:
   ...
   - **Advanced setup** — walks every option, step by step. Security keys are
     ...
     - **Basics** — domain/IP, timezone (`TZ`), HTTP host port (`MY_HUB_HTTP_PORT`)
...
(`MY_HUB_INSTALLER_LANG` > `LC_ALL` > `LC_MESSAGES` > `LANG`) and accepts a
