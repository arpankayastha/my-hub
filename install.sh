...
# der Umgebung (MY_HUB_INSTALLER_LANG > LC_ALL > LC_MESSAGES > LANG), analog der App.
...
resolve_locale() {
  normalize_locale "${MY_HUB_INSTALLER_LANG:-${LC_ALL:-${LC_MESSAGES:-${LANG:-}}}}"
}
...
configure_basic() {
  ...
  ask "$(t basic.host)"
  read -r MY_HUB_HOST; MY_HUB_HOST="${MY_HUB_HOST:-localhost}"

  ask "$(t basic.port)"
  read -r MY_HUB_PORT; MY_HUB_PORT="${MY_HUB_PORT:-3000}"
  ...
  read -r MY_HUB_TZ; MY_HUB_TZ="${MY_HUB_TZ:-$sys_tz}"
}
...
configure_calendar() {
  ...
  if [ "${want_google,,}" = "y" ]; then
    ...
    info "$(t calendar.redirect_hint "http://${MY_HUB_HOST}:${MY_HUB_PORT}/api/v1/calendar/google/callback")"
    ask "$(t calendar.client_id)"; read -r GOOGLE_CLIENT_ID
    ask "$(t calendar.client_secret)"; read -rs GOOGLE_CLIENT_SECRET; printf "\n"
    GOOGLE_REDIRECT_URI="http://${MY_HUB_HOST}:${MY_HUB_PORT}/api/v1/calendar/google/callback"
  fi
...
configure_document_storage() {
  ...
  if [ "${want_document_webdav,,}" = "y" ]; then
    ...
    DOCUMENT_STORAGE_WEBDAV_PATH="${DOCUMENT_STORAGE_WEBDAV_PATH:-my-hub-documents}"
  fi
  ...
  if [ "${want_document_google_drive,,}" = "y" ]; then
    info "$(t document_google_drive.redirect_hint "http://${MY_HUB_HOST}:${MY_HUB_PORT}/api/v1/documents/storage/google-drive/callback")"
    ...
    GOOGLE_DRIVE_REDIRECT_URI="http://${MY_HUB_HOST}:${MY_HUB_PORT}/api/v1/documents/storage/google-drive/callback"
  fi
}
...
review_and_confirm() {
  step "$(t review.step)"
  printf "\n"
  printf "  %-16s %s%s%s\n"  "$(t review.host)"     "$CYAN"   "$MY_HUB_HOST" "$RESET"
  printf "  %-16s %s%s%s\n"  "$(t review.port)"     "$CYAN"   "$MY_HUB_PORT" "$RESET"
  printf "  %-16s %s%s%s\n"  "$(t review.timezone)" "$CYAN"   "$MY_HUB_TZ"   "$RESET"
...
ENVEOF
  ...
  while [ $elapsed -lt 120 ]; do
    ...
    http_code=$(curl -s -o /dev/null -w "%{http_code}" \
      "http://localhost:${MY_HUB_PORT}/health" 2>/dev/null || echo "000")
...
create_admin() {
  ...
  response=$(curl -s -w "\n%{http_code}" \
    -X POST "http://localhost:${MY_HUB_PORT}/api/v1/auth/setup" \
    -H "Content-Type: application/json" \
    -d "$payload")
  http_code=$(printf '%s' "$response" | tail -n1)
  body=$(printf '%s' "$response" | head -n-1)

  local url="http://${MY_HUB_HOST}:${MY_HUB_PORT}"
...
run_noninteractive() {
  ...
  MY_HUB_PORT=$(grep -E '^PORT=' .env 2>/dev/null | cut -d= -f2- | head -n1)
  MY_HUB_PORT="${MY_HUB_PORT:-3000}"
  MY_HUB_HOST="localhost"
