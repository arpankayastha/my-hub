...
import { setPermissions, clearPermissions } from '/my-hub/permissions.js';
import { localApiFetch } from '/my-hub/local/api-router.js';

const API_BASE = '/api/v1';
const isLocalMode = typeof window !== 'undefined' && window.__MY_HUB_LOCAL_MODE__;
...
async function apiFetch(path, options = {}, _retried = false) {
  if (isLocalMode) {
    try {
      return await localApiFetch(path, options);
    } catch (err) {
      if (err.name === 'ApiError') throw err;
      throw new ApiError(err.message || 'offline', 0);
    }
  }
