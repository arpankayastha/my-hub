# My Hub — Umbrel App Store source

This folder is the **tracked source** for the My Hub entry in the official Umbrel
...
the new multi-arch index digest and opens/updates a single rolling `my-hub-update`
...
`docker buildx imagetools inspect ghcr.io/arpankayastha/my-hub:<version>` (top-level
`Digest:`) and bump `version`/`@sha256:` in a fork PR by hand.

## Config notes (why the compose looks like this)

- **`app_proxy`** is mandatory. My Hub has its own login, so `PROXY_AUTH_ADD: "false"`
  prevents a double sign-in. `APP_PORT: 3000` is where My Hub listens inside the
  ...
  **Security note:** with proxy auth off, My Hub's unauthenticated first-run
...
Then open <http://localhost>, finish onboarding, and sideload My Hub **before it is
merged** via a temporary Community App Store:

1. Create a throwaway public git repo with this layout:
   ```
   umbrel-app-store.yml       # id: my-hub-test, name: My Hub Test
   myhub/umbrel-app.yml      # copy of this folder's manifest
   myhub/docker-compose.yml  # copy of this folder's compose
   ```
2. In umbrelOS → App Store → "Community App Stores", add the repo URL.
3. Install My Hub, create the first account, then **restart the app** and confirm the
