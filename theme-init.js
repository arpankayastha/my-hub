// Einmalige, idempotente Migration aller Legacy-„myhub"-Storage-Keys → „myhub".
...
// Benennt jeden Key, der mit `my-hub-`, `myhub:` oder `myhub.` beginnt, auf das
// gleiche Suffix mit `myhub`-Präfix um (z. B. `my-hub-theme` → `my-hub-theme`).
  ...
  try {
    ...
    for (var s = 0; s < stores.length; s++) {
      ...
      for (var i = 0; i < store.length; i++) {
        var k = store.key(i);
        if (k && /^myhub[-:.]/.test(k)) keys.push(k);
      }
      for (var j = 0; j < keys.length; j++) {
        var oldKey = keys[j];
        var newKey = 'myhub' + oldKey.slice('myhub'.length);
...
(function() {
  var stored = localStorage.getItem('my-hub-theme');
  if (stored === 'dark') {
