...
import { t, whenI18nReady } from '/my-hub/i18n.js';
import { assetUrl } from '/my-hub/app-path.js';
import {
  getPwaInstallState,
  onPwaInstallStateChanged,
  promptPwaInstall,
} from '/my-hub/utils/pwa-install.js';

const DISMISS_KEY = 'my-hub-install-dismissed';
const DISMISS_DURATION_MS = 7 * 24 * 60 * 60 * 1000; // 7 Tage

const INTERACTION_KEY = 'my-hub-install-interactions';
const INTERACTION_THRESHOLD = 2;

class MyHubInstallPrompt extends HTMLElement {
  constructor() {
    super();
  ...
  async _onInstallClick() {
    try {
      ...
      console.log('[my-hub-install-prompt] Ergebnis:', result.outcome);

      if (result.outcome === 'accepted') {
        this._remove();
      }
    } catch (err) {
      console.error('[my-hub-install-prompt] Fehler:', err);
    }
    this._deferredPrompt = null;
  }
...
customElements.define('my-hub-install-prompt', MyHubInstallPrompt);
