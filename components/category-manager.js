...
import { makeSortable, isDragActive } from '/my-hub/utils/sortable.js';
...
class CategoryManagerElement extends HTMLElement {
  ...
  async _load() {
    try {
      ...
      window.myHub?.showToast(this._errMsg(err), 'danger');
    }
  }
  ...
  _warnDragUnavailable(err) {
    ...
    console.warn('[my-hub-category-manager] Drag-and-Drop nicht verfügbar (SortableJS-Import fehlgeschlagen); Auf/Ab-Buttons bleiben nutzbar.', err);
  }
  ...
  async _onSubmit(e) {
    ...
    try {
      ...
      window.myHub?.showToast(t('category.added'), 'success');
      this._notifyChanged();
    } catch (err) {
      window.myHub?.showToast(this._errMsg(err), 'danger');
    }
  ...
  async _rename(key) {
    ...
    if (!cat) return;
    const { promptModal } = await import('/my-hub/components/modal.js');
    ...
    try {
      ...
      window.myHub?.showToast(t('category.renamed'), 'success');
      this._notifyChanged();
    } catch (err) {
      window.myHub?.showToast(this._errMsg(err), 'danger');
    }
  ...
  async _delete(key) {
    const cat = this._cats.find((c) => this._keyOf(c) === key);
    if (!cat) return;
    const { confirmModal } = await import('/my-hub/components/modal.js');
    ...
    try {
      ...
      this._renderGroup(cat.type ?? cat.group ?? '');
      window.myHub?.showToast(t('category.deleted'), 'default');
      this._notifyChanged();
    } catch (err) {
      window.myHub?.showToast(this._errMsg(err), 'danger');
    }
...
customElements.define('my-hub-category-manager', CategoryManagerElement);
