...
function _resumeSuspendedModal({ overlay, id, snapshot }) {
  ...
  modalState = 'open';
  if (window.myHub?.setThemeColor) {
    window.myHub.setThemeColor(OVERLAY_THEME_COLOR, OVERLAY_THEME_COLOR);
  }
}
...
function _doClose(overlayEl) {
  ...
  if (activeOverlay === target) {
    ...
    if (window.myHub?.restoreThemeColor) {
      window.myHub.restoreThemeColor();
    }
  }
}
...
function _focusField(input) {
  // Custom Elements (z. B. my-hub-datepicker) sind selbst nicht fokussierbar:
