 ...
 * Abhängigkeiten: /api.js, /router.js (window.myHub)
 */

import { api } from '/my-hub/api.js';
import { renderRRuleFields, bindRRuleEvents, getRRuleValues } from '/my-hub/rrule-ui.js';
import { openModal as openSharedModal, closeModal, advancedSection, wireBlurValidation, reportFieldError } from '/my-hub/components/modal.js';
import { stagger, wireScrollFade, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { t, formatDate as formatPreferredDate, formatDayMonth, formatTime, timeSuffix, formatDateInput, parseDateInput, isDateInputValid, formatTimeInput, parseTimeInput } from '/my-hub/i18n.js';
import { esc, fmtLocation } from '/my-hub/utils/html.js';
import { shiftEndDateKey, isEndBeforeStart, weekStartIndex, weekdayOrder } from '/my-hub/utils/date.js';
import { truncateRuleBefore, shiftSeriesStart, shiftEndForStart } from '/my-hub/utils/recurrence-scope.js';
import { getReadableTextColor } from '/my-hub/utils/color.js';
import { refresh as refreshReminders } from '/my-hub/reminders.js';
import { parseRemindAtAsUtc } from '/my-hub/utils/reminder-offset.js';
import { renderUserMultiSelect, getSelectedUserIds, bindUserMultiSelect, renderAvatarStack } from '/my-hub/components/user-multi-select.js';
import { wireTablist } from '/my-hub/utils/tablist.js';
import { localizeBirthdayEvent } from '/my-hub/utils/birthday-event.js';
...
const CALENDAR_VIEW_STORAGE_KEY = 'myhub:calendar:view';
const LEGACY_CALENDAR_VIEW_STORAGE_KEY = 'my-hub-calendar-view';
const LAYER_HOLIDAYS_KEY = 'myhub:calendar:layer:holidays';
const LAYER_SCHOOL_KEY    = 'myhub:calendar:layer:school';
const ASSIGNED_TO_ME_KEY  = 'myhub:calendar:assignedToMe';
...
async function loadRange(from, to) {
  ...
  try {
    ...
    window.myHub?.showToast(t('calendar.loadError'), 'danger');
  }
...
async function getCachedAt(path) {
  ...
  try {
    ...
    const apiCache = names.find((n) => n.startsWith('my-hub-api-'));
...
function renderMonthView(container) {
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (!isMobile) {
      ...
      if (taskChip) {
        e.stopPropagation();
        window.myHub.navigate(`/tasks?open=${taskChip.dataset.taskId}`);
        return;
      }
  ...
  if (ev.external_source === 'ics' && ev.user_modified === 1) {
    ...
    resetLink.addEventListener('click', async (e) => {
      ...
      try {
        ...
        dismiss();
        await reloadForView();
        window.myHub?.showToast(t('calendar.ics.resetToast'), 'success');
      } catch (err) {
        // Server-Meldung bevorzugen (nutzerorientiert), sonst lokalisierter
        // Fallback — nie den rohen JS-/Netzwerk-Fehlertext zeigen.
        window.myHub?.showToast(err.data?.error ?? t('calendar.saveError'), 'danger');
      }
    });
...
function openEventModal({ mode, event = null, date = null, reminder = null, time = null }) {
  if (mode === 'edit' && event?.housekeeping_visit_id) {
    window.myHub.navigate(`/housekeeping?editVisit=${event.housekeeping_visit_id}`);
    return;
  }
    ...
    <div id="time-fields">
      <div class="modal-grid modal-grid--2">
        <div class="form-group">
          ...
          <my-hub-datepicker type="date" id="modal-start-date" value="${esc(formatDateInput(startDate))}" label="${esc(t('calendar.startDateLabel'))}"></my-hub-datepicker>
        </div>
        <div class="form-group">
          <label class="form-label" for="modal-start-time">${t('calendar.startTimeLabel')}</label>
          <my-hub-datepicker type="time" id="modal-start-time" value="${esc(formatTimeInput(startTime))}" label="${esc(t('calendar.startTimeLabel'))}"></my-hub-datepicker>
        </div>
      </div>
      <div class="modal-grid modal-grid--2">
        <div class="form-group">
          <label class="form-label" for="modal-end-date">${t('calendar.endDateLabel')}</label>
          <my-hub-datepicker type="date" id="modal-end-date" value="${esc(formatDateInput(endDate))}" label="${esc(t('calendar.endDateLabel'))}"></my-hub-datepicker>
        </div>
        <div class="form-group">
          <label class="form-label" for="modal-end-time">${t('calendar.endTimeLabel')}</label>
          <my-hub-datepicker type="time" id="modal-end-time" value="${esc(formatTimeInput(endTime))}" label="${esc(t('calendar.endTimeLabel'))}"></my-hub-datepicker>
        </div>
      </div>
    </div>
    ...
    <div id="allday-fields" style="display:none;">
      <div class="modal-grid modal-grid--2">
        <div class="form-group">
          ...
          <my-hub-datepicker type="date" id="modal-allday-start" value="${esc(formatDateInput(startDate))}" label="${esc(t('calendar.fromLabel'))}"></my-hub-datepicker>
        </div>
        <div class="form-group">
          <label class="form-label" for="modal-allday-end">${t('calendar.toLabel')}</label>
          <my-hub-datepicker type="date" id="modal-allday-end" value="${esc(formatDateInput(endDate))}" label="${esc(t('calendar.toLabel'))}"></my-hub-datepicker>
        </div>
      </div>
    </div>
...
async function saveEvent(overlay, mode, event, existingReminder = null, attachmentState = null) {
  ...
  try {
    ...
    closeModal({ force: true });
    renderView();
    window.myHub?.showToast(mode === 'create' ? t('calendar.createdToast') : t('calendar.savedToast'), 'success');
  } catch (err) {
...
async function deleteEvent(id) {
  ...
  scheduleUndoableDelete({
    ...
    restore: (err) => {
      ...
      if (err) window.myHub?.showToast(err.data?.error ?? t('calendar.deleteError'), 'danger');
    },
  });
}
