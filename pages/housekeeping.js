...
async function toggleSession(container, workerId) {
  ...
  if (!state.workers.length) {
    window.myHub?.showToast(t('housekeeping.checkInDisabled'), 'warning');
    return;
  }
  if (!worker) return;
  try {
    if (current) {
      ...
      window.myHub?.showToast(t('housekeeping.checkedOutToast'), 'success');
    } else {
      ...
      window.myHub?.showToast(t('housekeeping.checkedInToast'), 'success');
    }
    await loadData();
...
async function createTask(payload, content) {
  try {
    ...
    window.myHub?.showToast(t('housekeeping.taskCreatedToast'), 'success');
    await loadData();
    renderTasks(content);
  } catch (err) {
    window.myHub?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-complete-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try {
        ...
        window.myHub?.showToast(t('housekeeping.taskDoneToast'), 'success');
        await loadData();
        renderTasks(content);
      } catch (err) {
        window.myHub?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-undo-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try {
        ...
        window.myHub?.showToast(t('housekeeping.taskUndoneToast'), 'success');
        await loadData();
        renderTasks(content);
      } catch (err) {
        window.myHub?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-delete-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('housekeeping.taskDeletedToast'), 'success');
        await loadData();
        renderTasks(content);
      } catch (err) {
        window.myHub?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-pay-report]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('housekeeping.visitPaidToast'), 'success');
        await loadData();
  ...
  content.querySelectorAll('[data-delete-visit]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('housekeeping.visitDeletedToast'), 'success');
        await loadData();
        await loadStaffVisits();
        renderStaff(content);
      } catch (err) {
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-task-edit-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('housekeeping.taskUpdatedToast'), 'success');
          await loadData();
          closeModal({ force: true });
          renderTasks(content);
        } catch (err) {
    ...
    content: `
      <form id="housekeeping-visit-form" class="housekeeping-worker-form">
        <label class="housekeeping-field">
          ...
          <my-hub-datepicker name="date" type="date" value="${esc(visit.check_in.slice(0, 10))}"></my-hub-datepicker>
        </label>
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-visit-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('housekeeping.visitSavedToast'), 'success');
          await loadData();
...
function openStaffModal(worker, content, options = {}) {
      ...
      <form id="housekeeping-worker-form" class="housekeeping-worker-form">
        ...
        <div class="housekeeping-form-grid housekeeping-form-grid--wide">
          ...
          <label class="housekeeping-field">
            ...
            <my-hub-datepicker name="birth_date" type="date" value="${esc(item.birth_date || '')}"></my-hub-datepicker>
          </label>
          <label class="housekeeping-field">
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-worker-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('housekeeping.workerSavedToast'), 'success');
          await loadData();
          closeModal({ force: true });
  ...
  avatarFile?.addEventListener('change', async () => {
    ...
    try {
      ...
      const { openCropDialog } = await import('/my-hub/utils/avatar-crop.js');
