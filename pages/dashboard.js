...
import { canSeeWidget } from '/my-hub/permissions.js';
import { t, formatDate, formatTime, timeSuffix, getLocale, getNumberFormat } from '/my-hub/i18n.js';
import { getReadableTextColor, AVATAR_FALLBACK_COLOR } from '/my-hub/utils/color.js';
import { esc, fmtLocation, renderMarkdownLight } from '/my-hub/utils/html.js';
import { toLocalDateKey, parseLocalDateKey } from '/my-hub/utils/date.js';
import { predictCycle, PHASE } from '/my-hub/utils/health-cycle.js';
import { localizeBirthdayEvent } from '/my-hub/utils/birthday-event.js';
import { openModal, closeModal, confirmModal } from '/my-hub/components/modal.js';
import { renderAvatarStack } from '/my-hub/components/user-multi-select.js';
...
import { profileDisplayName } from '/my-hub/utils/profile-context.js';
import { vitalMetric } from '/my-hub/utils/health-vitals.js';
...
function getAppName() {
  return localStorage.getItem(APP_NAME_STORAGE_KEY) || 'My Hub';
}
...
function isWidgetModuleEnabled(id) {
  ...
  if (mod && window.myHub?.isModuleDisabled(mod)) return false;
...
function renderTodayCockpit(data, cfg = []) {
  ...
  const showCard = (module) => !window.myHub?.isModuleDisabled(module) && !widgetShown(module) && hasContent[module];
...
function initFab(container, signal) {
  ...
  fabActions.querySelectorAll('[data-route]').forEach((el) => {
    const go = async () => {
      toggleFab(false);
      await window.myHub.navigate(el.dataset.route);
    ...
    onSave: (panel) => {
      panel.querySelector('[data-action="done"]').addEventListener('click', async () => {
        try {
          ...
          window.myHub?.showToast(t('tasks.swipedDoneToast'), 'success');
          rerender();
        } catch (err) {
      ...
      panel.querySelector('[data-action="edit"]').addEventListener('click', () => {
        closeModal({ force: true });
        window.myHub.navigate(`/tasks?open=${taskId}`);
      });
    },
  });
}
...
function wireLinks(container, rerender, { editing = false } = {}) {
  container.querySelectorAll('[data-route]').forEach((el) => {
    ...
    const go = () => window.myHub.navigate(el.dataset.route);
    if (el.tagName === 'A') {
...
export async function render(container, { user }) {
  ...
  async function ensureCycleSlice() {
    ...
    if (window.myHub?.isModuleDisabled('health')) return;
    try {
    ...
    const onUndo = changed
      ? async () => {
          try {
            ...
            window.myHub?.showToast(t('common.errorGeneric'), 'danger');
          }
          isCustomizing = false;
          rebuildDashboard(widgetConfig);
        }
      : null;
    window.myHub?.showToast(t('dashboard.customizeSaved'), 'success', onUndo ? 6000 : 1500, onUndo);
  }
  ...
  const doWeatherRefresh = async () => {
    ...
    try {
      ...
      if (wWidget) {
        ...
        onUpdated?.(res.data);
        window.myHub?.showToast(t('dashboard.weatherUpdated'), 'success', 1500);
      }
    } catch {
      window.myHub?.showToast(t('common.errorGeneric'), 'danger');
    } finally {
