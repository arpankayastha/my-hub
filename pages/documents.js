...
import { openModal as openSharedModal, closeModal, selectModal, advancedSection, promptModal, confirmModal } from '/my-hub/components/modal.js';
import { t, formatDate, getLocale } from '/my-hub/i18n.js';
...
let state = {
  ...
  view: localStorage.getItem('my-hub-documents-view') || 'grid',
  sort: SORTS.includes(localStorage.getItem('my-hub-documents-sort'))
    ? localStorage.getItem('my-hub-documents-sort')
    : 'updated',
  status: 'active',
  category: '',
  folderId: '',
...
function bindPageEvents() {
  ...
  _container.querySelector('#documents-sort')?.addEventListener('change', (e) => {
    ...
    localStorage.setItem('my-hub-documents-sort', state.sort);
    applyFilters();
    renderDocuments();
  });
  ...
  _container.querySelector('.documents-view-toggle')?.addEventListener('click', (e) => {
    ...
    localStorage.setItem('my-hub-documents-view', state.view);
...
async function renameFolder(folder) {
  ...
  try {
    ...
    window.myHub?.showToast(t('documents.folderRenamedToast'), 'success');
...
async function deleteFolder(folder) {
  ...
  try {
    ...
    window.myHub?.showToast(t('documents.folderDeletedToast'), 'default');
...
async function runDocumentAction(action, doc) {
  ...
  if (action === 'archive') {
    ...
    window.myHub?.showToast(doc.status === 'archived' ? t('documents.restoredToast') : t('documents.archivedToast'), 'success');
    await loadDocuments();
    renderAll();
  }
  if (action === 'push-dms') {
    ...
    try {
      ...
      window.myHub?.showToast(t('documents.pushToDmsQueued'), 'success');
    } catch (err) {
      window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
    }
    return;
...
async function moveSelected() {
  ...
  try {
    ...
    window.myHub?.showToast(t('documents.bulkMovedToast', { count: docs.length }), 'success');
    await loadDocuments();
    renderAll();
  } catch (err) {
    window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
...
async function saveDocument(event, doc, panel) {
  ...
  try {
    ...
    if (doc) {
      ...
      window.myHub?.showToast(t('documents.savedToast'), 'success');
    } else {
    ...
    onSave(panel) {
      panel.querySelector('#document-folder-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('documents.folderCreatedToast'), 'success');
...
function renderPdfPages(container, previewUrl, doc, downloadUrl) {
  ...
  (async () => {
    try {
      const pdfjs = await import('/my-hub/vendor/pdfjs/pdf.min.mjs');
      if (state.destroyed) return;
