...
import { openModal as openSharedModal, closeModal, wireBlurValidation, validateAll, btnSuccess, btnError, promptModal, advancedSection } from '/my-hub/components/modal.js';
import { stagger, vibrate, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { t, formatDate, formatTime, formatDateInput, parseDateInput, isDateInputValid, formatTimeInput, parseTimeInput } from '/my-hub/i18n.js';
...
import { resolveReminderPreset } from '/my-hub/utils/reminder-offset.js';
import { renderPageSearch, wirePageSearch } from '/my-hub/utils/page-search.js';
import '/my-hub/components/category-manager.js';
  ...
  const advancedFieldsHtml = `
      ...
      <div class="modal-grid modal-grid--2" style="margin-top:var(--space-4)">
        <div class="form-group">
          ...
          <my-hub-datepicker type="date" id="task-start-date" name="start_date"
                 value="${esc(formatDateInput(task?.start_date))}"></my-hub-datepicker>
        </div>
        <div class="form-group">
    ...
    <form id="task-form" novalidate>
      ...
      <div class="modal-grid modal-grid--2">
        <div class="form-group">
          ...
          <my-hub-datepicker type="date" id="task-due-date" name="due_date"
                 value="${esc(formatDateInput(task?.due_date))}"></my-hub-datepicker>
        </div>
        <div class="form-group">
          <label class="label" for="task-due-time">${t('tasks.dueTimeLabel')}</label>
          <my-hub-datepicker type="time" id="task-due-time" name="due_time"
                 value="${esc(formatTimeInput(task?.due_time ?? ''))}"></my-hub-datepicker>
        </div>
      </div>
...
async function handleFormSubmit(e, container) {
  ...
  try {
    ...
    if (taskId) {
      ...
      window.myHub.showToast(t('tasks.savedToast'), 'success');
    } else {
      const res = await api.post('/tasks', body);
      savedTaskId = res.data?.id;
      window.myHub.showToast(t('tasks.createdToast'), 'success');
    }
...
async function handleDeleteTask(id, container) {
  ...
  scheduleUndoableDelete({
    ...
    restore: (err) => {
      ...
      if (err) window.myHub.showToast(err.message ?? t('common.unknownError'), 'danger');
    },
  });
}
...
async function handleAddSubtask(parentId, container) {
  ...
  try {
    ...
    window.myHub.showToast(err.message, 'danger');
  }
}
  ...
  board.addEventListener('click', async (e) => {
    ...
    if (e.target.closest('[draggable]')) {
      ...
      try {
        ...
        window.myHub.showToast(t('tasks.loadError'), 'danger');
      }
    }
  });
}
...
const RECENT_FILTERS_KEY = 'myhub:recentTaskFilters';
const RECENT_FILTERS_MAX = 3;
const SHOW_FUTURE_KEY = 'myhub:taskShowFuture';
const ASSIGNED_TO_ME_KEY = 'myhub:taskAssignedToMe';
...
function wireViewToggle(container) {
  ...
  toggle.querySelectorAll('[data-view]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      localStorage.setItem('my-hub-tasks-view', state.viewMode);
      renderFilters(container);
  ...
  bar.addEventListener('click', async (e) => {
    ...
    try {
      if (action === 'bulk-mark-done' || action === 'bulk-mark-open') {
        ...
        window.myHub.showToast(t('tasks.bulkStatusChanged'), 'success');
      } else if (action === 'bulk-archive') {
        await Promise.all(taskIds.map(id => api.patch(`/tasks/${id}/status`, { status: 'archived' })));
        window.myHub.showToast(t('tasks.bulkArchived'), 'success');
      }
      ...
      window.myHub.showToast(err.message ?? t('common.errorGeneric'), 'danger');
    }
  });
}
  ...
  listEl.addEventListener('click', async (e) => {
    ...
    if (action === 'archive-task') {
      try {
        ...
        window.myHub.showToast(t('tasks.archivedToast'), 'success');
        await loadTasks(container);
      } catch (err) {
        window.myHub.showToast(err.message, 'danger');
      }
...
export async function render(container, { user }) {
  ...
  const savedView = localStorage.getItem('my-hub-tasks-view');
