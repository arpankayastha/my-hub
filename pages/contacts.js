...
import { openModal as openSharedModal, closeModal, advancedSection } from '/my-hub/components/modal.js';
import { stagger, vibrate, wireScrollFade, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { t, formatDate } from '/my-hub/i18n.js';
...
import { parseVCards } from '/my-hub/utils/vcard.js';
import { composeDisplayName, contactSortKey, splitDisplayName } from '/my-hub/utils/contact-name.js';
import { getPhoneFormatter, createAsYouType, countryFromRegion } from '/my-hub/utils/phone.js';
...
export async function render(container, { user }) {
  ...
  _container.querySelector('#contacts-import-input').addEventListener('change', async (e) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('contacts.importError', { error: err.message }), 'danger');
      return;
    }
    ...
    if (named.length === 0) { window.myHub?.showToast(t('contacts.vcardNoName'), 'warning'); return; }
...
async function openContactModal({ mode, contact = null }) {
  ...
  const advancedFieldsHtml = `
    ...
    <div class="form-group">
      ...
      <my-hub-datepicker id="cm-birthday" type="date" value="${v('birthday')}"></my-hub-datepicker>
  ...
  openSharedModal({
    ...
    onSave(panel) {
      ...
      panel.querySelector('#cm-save').addEventListener('click', async () => {
        ...
        if (!name) {
          window.myHub?.showToast(t('contacts.nameRequiredHint'), 'danger');
        ...
        try {
          ...
          closeModal({ force: true });
          renderList();
          window.myHub?.showToast(mode === 'create' ? t('contacts.savedToast') : t('contacts.updatedToast'), 'success');
        } catch (err) {
...
function openBirthdayImport() {
  try { sessionStorage.setItem('myhub:birthdays:autoImport', '1'); } catch { /* egal */ }
  window.myHub?.navigate('/birthdays');
}
...
async function importParsedContacts(list) {
  ...
  if (imported === 0) {
    ...
    const reason = window.myHub?.friendlyError?.(lastError) || lastError?.message || '';
  ...
  window.myHub?.showToast(message, type, action ? 6000 : 3000, action);
}
