...
import { promptModal, openModal, closeModal, confirmModal } from '/my-hub/components/modal.js';
import { DEFAULT_CATEGORY_NAME, categoryLabel } from '/my-hub/utils/shopping-categories.js';
import { addLocalDays, toLocalDateKey } from '/my-hub/utils/date.js';
...
async function openPantryTransfer(container) {
  ...
  const { PANTRY_UNITS } = await import('/my-hub/utils/pantry-units.js');
  const { locationLabel } = await import('/my-hub/utils/pantry-locations.js');
...
async function openCategoryManager(container, { fromDeepLink = false } = {}) {
  const { openModal } = await import('/my-hub/components/modal.js');
