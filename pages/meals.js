...
import { openModal as openSharedModal, closeModal as closeSharedModal, selectModal, confirmModal, advancedSection, wireBlurValidation, reportFieldError } from '/my-hub/components/modal.js';
import { stagger, scheduleUndoableDelete, wireScrollFade } from '/my-hub/utils/ux.js';
import { t, formatDate, formatDayMonth, formatDateInput, parseDateInput, isDateInputValid } from '/my-hub/i18n.js';
...
import { addLocalDays, startOfLocalWeekKey, toLocalDateKey } from '/my-hub/utils/date.js';
import { normalizeRecipeMealTypes, recipeSupportsMealType } from '/my-hub/utils/recipe-meal-types.js';
import { mountEmptyState, emptyStateEl } from '/my-hub/utils/empty-state.js';
...
async function loadWeek(week) {
  try {
    ...
    window.myHub?.showToast(t('meals.loadError'), 'danger');
  }
}
...
const RAIL_STORAGE_KEY = 'my-hub-meals-rail';
  ...
  if (!state.recipes.length) {
    ...
    sidebar.appendChild(emptyStateEl({
      ...
      action: {
        ...
        icon: 'plus',
        onClick: () => window.myHub?.navigate('/recipes'),
      },
    }));
...
async function addRecipeToSlot(recipe, date, mealType, { replaceMeals = [] } = {}) {
  try {
    ...
    window.myHub?.showToast(window.myHub?.friendlyError?.(err) ?? t('common.errorGeneric'), 'danger');
  }
}
...
async function runRandomize(panel) {
  ...
  try {
    ...
    renderWeekGrid();
    window.myHub?.showToast(t('meals.randomizeSuccess', { count: plan.assignments.length }), 'success');
  } catch (err) {
    runBtn.disabled = false;
    window.myHub?.showToast(window.myHub?.friendlyError?.(err) ?? t('common.errorGeneric'), 'danger');
  }
    ...
    onSave(panel) {
      ...
      saveAsRecipeBtn?.addEventListener('click', async () => {
        ...
        try {
          ...
          window.myHub?.showToast(t('recipes.created'), 'success');
        } catch (err) {
      ...
      panel.querySelector('#transfer-btn')?.addEventListener('click', async () => {
        ...
        try {
          ...
          if (res.data.transferred > 0) {
            window.myHub?.showToast(t('meals.transferSuccess', { count: res.data.transferred }), 'success');
            await loadWeek(state.currentWeek);
            closeModal({ force: true });
            renderWeekGrid();
          } else {
            window.myHub?.showToast(t('meals.transferAlreadyDone'), 'info');
            btn.disabled = false;
          }
        } catch (err) {
  ...
  return `
    <div class="modal-grid modal-grid--2">
      <div class="form-group">
        ...
        <my-hub-datepicker type="date" id="modal-date" value="${formatDateInput(date)}"></my-hub-datepicker>
      </div>
      <div class="form-group">
...
async function saveModal(overlay) {
  ...
  try {
    ...
    closeModal({ force: true });
    renderWeekGrid();
    window.myHub?.showToast(mode === 'create' ? t('meals.addMealTitle') : t('meals.editMeal'), 'success');
  } catch (err) {
    window.myHub?.showToast(err.data?.error ?? t('common.errorGeneric'), 'danger');
  ...
  if (meal?.recurrence_template_id) {
    ...
    if (choice === 'series') {
      try {
        ...
        await loadWeek(state.currentWeek);
        renderWeekGrid();
        window.myHub?.showToast(t('meals.seriesDeletedToast'), 'success');
      } catch (err) {
        window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
      }
      return;
