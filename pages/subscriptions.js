...
import { closeModal, confirmModal, openModal, advancedSection, reportFieldError } from '/my-hub/components/modal.js';
import {
  formatDate,
  getLocale,
  getNumberFormat,
...
async function reload(options) {
  try {
    ...
    window.myHub?.showToast(err.data?.error || t('subscriptions.loadError'), 'danger');
  }
}
      ...
      <section class="subscription-form__section">
        ...
        <div class="form-grid-2">
          <div class="form-group">
            ...
            <my-hub-datepicker id="subscription-next-date" type="date"
                   value="${esc(subscription?.next_payment_date || toLocalDateKey(new Date()))}"></my-hub-datepicker>
          </div>
          <div class="form-group">
        ...
        <div class="form-grid-2">
          <div class="form-group" id="subscription-end-date-field" ${(subscription?.end_type === 'on_date') ? '' : 'hidden'}>
            ...
            <my-hub-datepicker id="subscription-end-date" type="date"
                   value="${esc(subscription?.end_date || '')}"></my-hub-datepicker>
          </div>
    ...
    onSave(panel) {
      ...
      panel.querySelector('#subscription-find-logo').addEventListener('click', async () => {
        openLogoPickerModal(panel, subscription?.website_url || panel.querySelector('#subscription-name').value.trim(), (logoData) => {
          ...
          showLogo(searchedLogoData);
          window.myHub?.showToast(t('subscriptions.logoFound'), 'success');
        });
      });
...
async function saveSubscription(panel, existing, searchedLogoData = null) {
  ...
  try {
    ...
    await reload();
    window.myHub?.showToast(t(existing ? 'subscriptions.savedToast' : 'subscriptions.addedToast'), 'success');
  } catch (err) {
    window.myHub?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
  } finally {
...
function openLogoPickerModal(panel, initialQuery, onSelect) {
  ...
  const search = async () => {
    ...
    try {
      ...
      window.myHub?.showToast(message, 'danger');
    } finally {
...
async function toggleSubscription(subscription) {
  try {
    ...
    await reload();
    window.myHub?.showToast(t(subscription.enabled ? 'subscriptions.disabledToast' : 'subscriptions.enabledToast'), 'success');
  } catch (err) {
...
async function renewSubscription(subscription) {
  try {
    ...
    window.myHub?.showToast(t(completed ? 'subscriptions.completedToast' : 'subscriptions.renewedToast'), 'success');
  } catch (err) {
    window.myHub?.showToast(err.data?.error || t('common.unknownError'), 'danger');
  }
}
...
async function deleteSubscription(subscription) {
  ...
  try {
    ...
    await reload();
    window.myHub?.showToast(t('subscriptions.deletedToast'), 'success');
  } catch (err) {
    window.myHub?.showToast(err.data?.error || t('common.unknownError'), 'danger');
  }
}
    ...
    onSave(panel) {
      ...
      panel.querySelector('#subscriptions-settings-form').addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('subscriptions.settingsSaved'), 'success');
        } catch (err) {
          window.myHub?.showToast(err.data?.error || t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelectorAll('[data-act="save"]').forEach((button) => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            await reload();
            openMetadataModal();
            window.myHub?.showToast(t('subscriptions.metaSavedToast'), 'success');
          } catch (err) {
            window.myHub?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
          }
        });
        ...
        button.addEventListener('click', async () => {
          ...
          if (confirmed) {
            try {
              ...
              await reload();
              window.myHub?.showToast(t('subscriptions.metaDeletedToast'), 'success');
            } catch (err) {
              window.myHub?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
            }
          }
