...
import { auth } from '/my-hub/api.js';
import { getLocale } from '/my-hub/i18n.js';
import { toAppUrlWithQuery, fromAppUrl } from '/my-hub/app-path.js';
import {
  SETTINGS_STORAGE_KEY,
  filterSettingsDomains,
  findSettingsLeaf,
  readStoredSettingsDestination,
} from '/my-hub/settings/registry.js';
import { renderSettingsShell } from '/my-hub/settings/shell.js';
...
function redirectTo(target) {
  ...
  setTimeout(() => {
    window.myHub?.navigate(target, false);
  }, 0);
}
...
export async function render(container, { user } = {}) {
  try {
    ...
    if (!leaf) {
      sessionStorage.setItem('myhub:settings:notice', 'accessRedirected');
