...
import { openModal as openSharedModal, closeModal, confirmModal, advancedSection, wireBlurValidation, reportFieldError } from '/my-hub/components/modal.js';
import { stagger, vibrate, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { wireTablist } from '/my-hub/utils/tablist.js';
import { t, formatDate, getLocale, getNumberFormat } from '/my-hub/i18n.js';
import { esc } from '/my-hub/utils/html.js';
import { renderSkeletonList } from '/my-hub/utils/skeleton.js';
import { render as renderSplitExpenses } from '/my-hub/pages/split-expenses.js';
import { openSubscriptionModal, render as renderSubscriptions } from '/my-hub/pages/subscriptions.js';
import { renderStats } from '/my-hub/pages/budget-stats.js';
import { renderPlans } from '/my-hub/pages/budget-plans.js';
import { toLocalDateKey } from '/my-hub/utils/date.js';
import { budgetCategoryLabel } from '/my-hub/utils/category-labels.js';
import { englishBudgetSubcategoryLabel } from '/my-hub/utils/budget-category-defaults.js';
import { appendCurrencyOptions } from '/my-hub/settings/currency.js';
import { wireHorizontalSwipe } from '/my-hub/utils/horizontal-swipe.js';
...
async function loadMonth(month) {
  ...
  try {
    ...
    window.myHub?.showToast(t('budget.loadError'), 'danger');
  }
}
...
async function loadBudgetMeta() {
  try {
    ...
    window.myHub?.showToast(t('budget.metaLoadError'), 'danger');
  }
}
    ...
    onSave(panel) {
      ...
      panel.querySelector('#am-archive')?.addEventListener('click', async () => {
        ...
        try {
          ...
          await loadAccounts();
          renderBody();
          window.myHub?.showToast(nextArchived ? t('budget.accountArchivedToast') : t('budget.accountRestoredToast'), 'success');
        } catch (err) {
      ...
      panel.querySelector('#am-delete')?.addEventListener('click', async () => {
        ...
        try {
          ...
          await loadMonth(state.month);
          renderBody();
          window.myHub?.showToast(t('budget.accountDeletedToast'), 'success');
        } catch (err) {
          window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelector('#am-save').addEventListener('click', async () => {
        ...
        try {
          ...
          await loadAccounts();
          renderBody();
          window.myHub?.showToast(isEdit ? t('budget.accountSavedToast') : t('budget.accountAddedToast'), 'success');
        } catch (err) {
    ...
    onSave(panel) {
      ...
      const addCategory = async () => {
        ...
        try {
          ...
          window.myHub?.showToast(t('budget.categoryAddedToast'), 'success');
        } catch (err) {
          window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      };
      ...
      const addSubcategory = async () => {
        ...
        try {
          ...
          window.myHub?.showToast(t('budget.subcategoryAddedToast'), 'success');
        } catch (err) {
          window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      };
      ...
      panel.querySelector('#bm-save').addEventListener('click', async () => {
        ...
        try {
          ...
          if (mode === 'create') {
            ...
            closeModal({ force: true });
            renderBody();
            window.myHub?.showToast(t('budget.addedToast'), 'success');
            ...
            if (scope === 'series') {
              ...
              window.myHub?.showToast(t('budget.savedToast'), 'success');
            }
            await loadMonth(state.month);
            renderBody();
...
async function saveLoanFromPanel(panel, saveBtn, { loan = null, closeAfterSave = false } = {}) {
  ...
  try {
    ...
    renderBody();
    window.myHub?.showToast(isEdit ? t('budget.loanSavedToast') : t('budget.loanAddedToast'), 'success');
  } catch (err) {
    window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
    saveBtn.disabled = false;
...
async function markLoanPayment(id) {
  ...
  try {
    ...
    if (paymentId) {
      window.myHub?.showToast(t('budget.loanPaymentAddedToast'), 'default', 5000, async () => {
        try {
      ...
      window.myHub?.showToast(t('budget.loanPaymentAddedToast'), 'success');
    }
  } catch (err) {
    window.myHub?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
  }
