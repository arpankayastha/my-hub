...
import { stagger, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { t, formatDate, parseDateInput, isDateInputValid } from '/my-hub/i18n.js';
...
function bindEvents() {
  ...
  try {
    if (sessionStorage.getItem('myhub:birthdays:autoImport')) {
      sessionStorage.removeItem('myhub:birthdays:autoImport');
      openImportModal();
    }
        ...
        <div class="birthday-modal__identity">
          ...
          <div class="birthday-modal__fields">
            ...
            <div class="form-group">
              ...
              <my-hub-datepicker id="bd-birth-date" type="date" max="${today}" value="${esc(birthday?.birth_date || '')}"></my-hub-datepicker>
            </div>
          </div>
        </div>
        ${advancedSection(`
    ...
    onSave(panel) {
      ...
      panel.querySelector('#bd-save').addEventListener('click', async () => {
        ...
        if (!body.name || !body.birth_date || !isDateInputValid(birthDateRaw)) {
          window.myHub?.showToast(t('birthdays.requiredFields'), 'warning');
          return;
        }
        ...
        try {
          if (isEdit) {
            ...
            window.myHub?.showToast(t('birthdays.updatedToast'), 'success');
          } else {
            await api.post('/birthdays', body);
            window.myHub?.showToast(t('birthdays.createdToast'), 'success');
          }
          await loadData();
          renderList();
  ...
  openSharedModal({
    ...
    onSave(panel) {
      ...
      submitBtn.addEventListener('click', async () => {
        ...
        if (ids.length === 0) {
          window.myHub?.showToast(t('birthdays.importNothingSelected'), 'warning');
          return;
        }
        submitBtn.disabled = true;
        try {
          const res = await api.post('/birthdays/import', { contact_ids: ids });
          window.myHub?.showToast(t('birthdays.importSuccess', { count: res.data.imported }), 'success');
          await loadData();
          renderList();
          closeModal({ force: true });
        } catch (err) {
