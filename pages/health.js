...
import { fromAppUrl } from '/my-hub/app-path.js';
import { effectiveUserId } from '/my-hub/utils/profile-context.js';
import {
  dashboardProfileToolMarkup,
  wireDashboardProfileTool,
} from '/my-hub/components/profile-switcher.js';
import { t, formatDate, formatTime, getLocale, getNumberFormat } from '/my-hub/i18n.js';
import { esc } from '/my-hub/utils/html.js';
import { downloadApiFile } from '/my-hub/utils/api-download.js';
import { wireScrollFade, scheduleUndoableDelete } from '/my-hub/utils/ux.js';
import { toLocalDateKey, parseLocalDateKey, addLocalDays } from '/my-hub/utils/date.js';
import { openModal, closeModal, confirmModal, reportFieldError, modalFormSubmitButton } from '/my-hub/components/modal.js';
import { createPageFab, setPageFabAction } from '/my-hub/utils/fab.js';
import { computeVitalSeries, VITAL_METRICS, vitalMetric } from '/my-hub/utils/health-vitals.js';
import {
  computeDueDoses, computeAdherence, refillState,
  daysMaskToIndices, indicesToDaysMask, WEEKDAY_COUNT,
} from '/my-hub/utils/health-meds.js';
import {
  deriveFlag, summarizeReport, analyteNames, analyteTrend, LAB_FLAGS,
} from '/my-hub/utils/health-labs.js';
import {
  ACTIVITY_TYPES, activityType, weekSummary, activityTotals,
} from '/my-hub/utils/health-activity.js';
import { upcomingDoses, computeAdherenceStreak } from '/my-hub/utils/health-overview.js';
import {
  FLOW_LEVELS, flowLevel, SYMPTOM_TYPES, symptomType, MOOD_TYPES, PHASE,
  predictCycle, cycleStats, buildCycleCalendar, cycleRing,
} from '/my-hub/utils/health-cycle.js';
import { HEALTH_ROUTES, renderHealthTabsBar } from '/my-hub/utils/health-tabs.js';
      ...
      <form id="vital-form" class="form-stack">
        ...
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <my-hub-datepicker id="vital-measured-at" type="datetime" value="${esc(localDateTimeValue(now))}"></my-hub-datepicker>
          </div>
          <div class="form-field">
...
async function handleDose(btn, action) {
  ...
  try {
    ...
    window.myHub?.showToast(t('health.meds.doseSaved'), 'success');
    await reloadMeds();
  } catch (err) {
    console.error('[Health] dose error:', err);
    btn.disabled = false;
    window.myHub?.showToast(err?.data?.error || t('health.meds.doseError'), 'danger');
  }
}
    ...
    onSave(panel) {
      ...
      panel.querySelector('#med-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('health.meds.saved'), 'success');
          await reloadMeds();
        } catch (err) {
          console.error('[Health] med save error:', err);
          submitBtn.disabled = false;
          window.myHub?.showToast(err?.data?.error || t('health.meds.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteMed(med) {
  ...
  try {
    ...
    closeModal({ force: true });
    window.myHub?.showToast(t('health.meds.deleted'), 'success');
    await reloadMeds();
  } catch (err) {
    console.error('[Health] med delete error:', err);
    window.myHub?.showToast(err?.data?.error || t('health.meds.deleteError'), 'danger');
  }
}
    ...
    <div class="health-sched-add">
      <div class="modal-grid modal-grid--2">
        <div class="form-field">
          ...
          <my-hub-datepicker id="sched-time" type="time" value="08:00"></my-hub-datepicker>
        </div>
        <div class="form-field">
      ...
      <form id="lab-form" class="form-stack">
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <my-hub-datepicker id="lab-date" type="date" value="${esc(dateValue)}"></my-hub-datepicker>
          </div>
          <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#lab-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.myHub?.showToast(t('health.labs.saved'), 'success');
          await reloadLabs();
        } catch (err) {
          console.error('[Health] lab save error:', err);
          submitBtn.disabled = false;
          window.myHub?.showToast(err?.data?.error || t('health.labs.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteLabReport(report) {
  ...
  try {
    ...
    closeModal({ force: true });
    window.myHub?.showToast(t('health.labs.deleted'), 'success');
    if (labs.selectedReportId === report.id) labs.selectedReportId = null;
    await reloadLabs();
  } catch (err) {
    console.error('[Health] lab delete error:', err);
    window.myHub?.showToast(err?.data?.error || t('health.labs.deleteError'), 'danger');
  }
}
  ...
  openModal({
    ...
    content: `
      <form id="activity-form" class="form-stack">
        ...
        <div class="form-field">
          ...
          <my-hub-datepicker id="activity-performed-at" type="datetime" value="${esc(dateValue)}"></my-hub-datepicker>
        </div>
        <div class="modal-grid modal-grid--3">
          <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#activity-form').addEventListener('submit', async (e) => {
        ...
        if (!body) {
          window.myHub?.showToast(t('health.activity.invalid'), 'danger');
          return;
        }
        submitBtn.disabled = true;
        try {
          ...
          closeModal({ force: true });
          window.myHub?.showToast(t('health.activity.saved'), 'success');
          await reloadActivity();
          await opts.onSaved?.();
        } catch (err) {
          console.error('[Health] activity save error:', err);
          submitBtn.disabled = false;
          window.myHub?.showToast(err?.data?.error || t('health.activity.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteActivity(row) {
  ...
  try {
    ...
    closeModal({ force: true });
    window.myHub?.showToast(t('health.activity.deleted'), 'success');
    await reloadActivity();
  } catch (err) {
    console.error('[Health] activity delete error:', err);
    window.myHub?.showToast(err?.data?.error || t('health.activity.deleteError'), 'danger');
  }
}
    ...
    <div class="health-overview__export">
      <div class="health-overview__export-range">
        <div class="form-field">
          ...
          <my-hub-datepicker id="ov-export-from" type="date" value="${esc(from || '')}"></my-hub-datepicker>
        </div>
        <div class="form-field">
          <label class="label" for="ov-export-to">${esc(t('health.export.rangeTo'))}</label>
          <my-hub-datepicker id="ov-export-to" type="date" value="${esc(to || '')}"></my-hub-datepicker>
        </div>
      </div>
...
function wireOverview() {
  ...
  overview.root.querySelectorAll('[data-vital-nav]').forEach((card) =>
    card.addEventListener('click', () => {
      ...
      window.myHub?.navigate('/health/vitals');
    }));
  ...
  overview.root.querySelector('[data-action="ov-go-meds"]')
    ?.addEventListener('click', () => window.myHub?.navigate('/health/meds'));
...
async function cycleStartPeriodToday() {
  ...
  try {
    ...
    cycle.anchor = today;
    window.myHub?.showToast(t('health.cycle.today.startedToast'), 'success');
    await reloadCycle();
  } catch (err) {
    console.error('[Health] cycle start error:', err);
    window.myHub?.showToast(err?.data?.error || t('health.cycle.saveError'), 'danger');
  }
}

async function cycleEndPeriodToday() {
  const open = cycleOpenPeriod();
  if (!open) { window.myHub?.showToast(t('health.cycle.today.noOpenPeriod'), 'info'); return; }
  try {
    await api.patch(`/health/cycle/periods/${open.id}`, { end_date: toLocalDateKey(new Date()) });
    window.myHub?.showToast(t('health.cycle.today.endedToast'), 'success');
    await reloadCycle();
  } catch (err) {
  ...
  openModal({
      ...
      <form id="cycle-period-form" class="form-stack">
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <my-hub-datepicker id="cycle-start" type="date" value="${esc(startVal)}"></my-hub-datepicker>
          </div>
          <div class="form-field">
            <label class="label" for="cycle-end">${esc(t('health.cycle.field.endDate'))}</label>
            <my-hub-datepicker id="cycle-end" type="date" value="${esc(endVal)}"></my-hub-datepicker>
          </div>
        </div>
        <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#cycle-period-form').addEventListener('submit', async (e) => {
        ...
        if (!start) { window.myHub?.showToast(t('health.cycle.invalid'), 'danger'); return; }
        if (end && end < start) { window.myHub?.showToast(t('health.cycle.invalid'), 'danger'); return; }
        const body = {
          start_date: start,
          end_date: end || null,
        ...
        try {
          ...
          closeModal({ force: true });
          window.myHub?.showToast(t('health.cycle.saved'), 'success');
          await reloadCycle();
        } catch (err) {
...
async function deletePeriod(period) {
  ...
  try {
    ...
    closeModal({ force: true });
    window.myHub?.showToast(t('health.cycle.deleted'), 'success');
    await reloadCycle();
  } catch (err) {
    console.error('[Health] cycle period delete error:', err);
    window.myHub?.showToast(err?.data?.error || t('health.cycle.deleteError'), 'danger');
  }
}
    ...
    content: `
      <form id="cycle-settings-form" class="form-stack">
        ...
        <div class="form-field" id="cs-due-field" ${s.pregnancy_mode ? '' : 'hidden'}>
          ...
          <my-hub-datepicker id="cs-due" type="date" value="${esc(s.pregnancy_due_date || '')}" min="${esc(dueMin)}" max="${esc(dueMax)}"></my-hub-datepicker>
        </div>
    ...
    onSave(panel) {
      ...
      bulkRun?.addEventListener('click', async () => {
        ...
        try {
          ...
          showBulk(false);
          window.myHub?.showToast(t('health.cycle.settings.applyToAllDone', { count }), 'success');
          await reloadCycle();
        } catch (err) {
      ...
      panel.querySelector('#cycle-settings-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.myHub?.showToast(t('health.cycle.settings.saved'), 'success');
          renderCycleShell();
        } catch (err) {
