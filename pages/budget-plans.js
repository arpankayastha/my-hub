...
import { openModal, closeModal, reportFieldError } from '/my-hub/components/modal.js';
import { vibrate } from '/my-hub/utils/ux.js';
...
function openAddPlan() {
  ...
  if (!options.length) {
    ...
    window.myHub?.showToast(t('budget.planAllCategoriesBudgeted'), 'default');
    return;
  }
...
async function savePlan(panel, category) {
  ...
  try {
    ...
    await load();
    window.myHub?.showToast(t('budget.planSavedToast'), 'success');
  } catch (err) {
...
async function deletePlan(category) {
  ...
  try {
    ...
    closeModal({ force: true });
    await load();
    window.myHub?.showToast(t('budget.planRemovedToast'), 'default', 5000, async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('common.unknownError'), 'danger');
      }
    });
  } catch (err) {
