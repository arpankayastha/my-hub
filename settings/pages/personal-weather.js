...
} from '/my-hub/settings/weather-location.js';
...
function bindEvents(container, user) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.personalWeatherSaved'), 'success');
  ...
  container.querySelector('#pweather-reset-btn')?.addEventListener('click', async () => {
    try {
      ...
      window.myHub?.showToast(t('settings.personalWeatherReset'), 'success');
      await render(container, { user });
    } catch (error) {
