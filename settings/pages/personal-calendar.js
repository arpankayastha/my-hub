...
function bindEvents(container) {
  ...
  assignMe?.addEventListener('change', async () => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.calendarDefaultsSaved'), 'success');
    } catch (error) {
  ...
  const persistReminders = debounce(async () => {
    ...
    try {
      ...
      persisted = selected;
      if (remindersBox.isConnected) window.myHub?.showToast(t('settings.calendarDefaultsSaved'), 'success');
    } catch (error) {
  ...
  remindersBox.addEventListener('change', (event) => {
    ...
    if (collectDefaultReminders(remindersBox).length > MAX_DEFAULT_REMINDERS) {
      ...
      window.myHub?.showToast(t('settings.calendarDefaultRemindersMax', { count: MAX_DEFAULT_REMINDERS }), 'warning');
      return;
    }
    persistReminders();
  });
