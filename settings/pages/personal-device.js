...
import { getPwaInstallState, onPwaInstallStateChanged, promptPwaInstall } from '/my-hub/utils/pwa-install.js';
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      if (result.outcome === 'accepted') {
        window.myHub?.showToast(t('settings.pwaInstallAcceptedToast'), 'success');
      } else if (result.outcome === 'ios') {
        window.myHub?.showToast(t('settings.pwaInstallIosToast'), 'default');
      } else if (result.outcome === 'installed') {
        window.myHub?.showToast(t('settings.pwaInstallAlreadyInstalledToast'), 'default');
      } else if (result.outcome === 'unavailable') {
        window.myHub?.showToast(t('settings.pwaInstallUnavailableToast'), 'warning');
      }
    } catch (error) {
