...
import { prefersInkText } from '/my-hub/utils/contrast.js';
import { openModal, closeModal, confirmModal } from '/my-hub/components/modal.js';
import { createRetryState, toggleRowHtml } from '/my-hub/settings/components.js';

import { FAMILY_ROLES, isValidFamilyRole } from '/my-hub/utils/family-roles.js';
      ...
      <div class="settings-card settings-card--hidden" id="add-member-form-card">
        ...
        <form id="add-member-form" class="settings-form">
          ...
          <div class="form-group">
            ...
            <my-hub-datepicker type="date" id="new-member-birth-date"></my-hub-datepicker>
  ...
  container.querySelectorAll('[data-delete-user]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('settings.memberDeletedToast', { name }), 'default');
      } catch (err) {
    ...
    content: `
      <form id="edit-member-form" class="settings-form">
        ...
        <div class="form-group">
          ...
          <my-hub-datepicker type="date" id="edit-member-birth-date" value="${esc(member.birth_date || '')}"></my-hub-datepicker>
          <p class="form-hint">${t('settings.memberContactBirthdayHint')}</p>
        </div>
        <div class="form-group">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#edit-member-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.myHub?.showToast(t('settings.memberUpdatedToast', { name: res.user.display_name }), 'success');
  ...
  if (addMemberForm) {
    addMemberForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        container.querySelector('#add-member-form-card').classList.add('settings-card--hidden');
        container.querySelector('#add-member-btn').hidden = false;
        window.myHub?.showToast(t('settings.memberAddedToast', { name: res.user.display_name }), 'success');
        bindDeleteButtons(container);
        bindEditButtons(container, currentUser, users);
      } catch (err) {
