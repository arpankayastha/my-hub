...
async function saveNavigationState(list, isAdmin) {
  ...
  if (isAdmin) {
    ...
    window.myHub?.setDisabledModules?.(savedDisabled);
  }
  window.myHub?.setModuleOrder?.(savedOrder);
...
function bindModuleListEvents(container, user) {
  ...
  const saveIfChanged = async (previousOrder) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.modulesSaved'), 'success');
    } catch (error) {
      ...
      await persistModuleToggle(
        ...
        async () => {
          if (input.dataset.thirdPartyModuleToggle) {
            ...
            await window.myHub?.refreshThirdPartyModules?.();
          }
          const savedPreferences = await saveNavigationState(list, isAdmin);
          window.myHub?.showToast(t('settings.thirdPartyModulesSaved'), 'success');
          return savedPreferences;
        },
  ...
  selects.forEach((changedSelect) => {
    changedSelect.addEventListener('change', async () => {
      ...
      try {
        ...
        window.myHub?.setMobileNavOrder?.(savedOrder);
        window.myHub?.showToast(t('settings.mobileNavigationSaved'), 'success');
        await render(container, { user });
      } catch (error) {
