...
import { appendCurrencyOptions, persistCurrencySelection } from '/my-hub/settings/currency.js';
import { getPreferences, savePreferences } from '/my-hub/settings/preferences-cache.js';
import {
  CUSTOM_REGION,
  REGION_CODES,
  REGION_PRESETS,
  ...
  regionLabel,
  numberLocaleFor,
} from '/my-hub/settings/region-presets.js';
...
function currentTheme() {
  return safeStorageGet('my-hub-theme', 'system') || 'system';
}
...
function localeOptions() {
  const storedLocale = safeStorageGet('my-hub-locale');
  return [
...
function applyTheme(value) {
  safeStorageSet('my-hub-theme', value);
  if (window.myHub?.applyTheme) {
    try {
      window.myHub.applyTheme(value);
      return;
    } catch {
...
function applyNumberLocale({ region, currency, date_format, time_format }) {
  ...
  if (numberLocale) {
    safeStorageSet('my-hub-number-locale', numberLocale);
  } else {
    safeStorageRemove('my-hub-number-locale');
  }
}
...
function bindEvents(container, user) {
  ...
  localeSelect?.addEventListener('change', async () => {
    ...
    try {
      if (localeSelect.value === 'system') {
        safeStorageRemove('my-hub-locale');
        location.reload();
        return;
      }
  ...
  regionSelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      safeStorageSet('my-hub-date-format', preset.date_format);
      safeStorageSet('my-hub-time-format', preset.time_format);
      applyNumberLocale({
        region: regionSelect.value,
        currency: preset.currency,
        date_format: preset.date_format,
      ...
      window.myHub?.showToast(t('settings.regionSaved'), 'success');
    } catch (error) {
      showError(errorElement, error.message);
    } finally {
  ...
  currencySelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.currencySaved'), 'success');
    } catch (error) {
      showError(errorElement, error.message);
    }
  });
  ...
  dateFormatSelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      safeStorageSet('my-hub-date-format', dateFormatSelect.value);
      window.dispatchEvent(new CustomEvent('date-format-changed', {
        detail: { dateFormat: dateFormatSelect.value },
      }));
      syncRegionSelect(container);
      applyNumberLocale(readFormatState(container));
      window.myHub?.showToast(t('settings.dateFormatSavedToast'), 'success');
    } catch (error) {
      showError(errorElement, error.message);
    } finally {
  ...
  timeFormatSelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      safeStorageSet('my-hub-time-format', timeFormatSelect.value);
      window.dispatchEvent(new CustomEvent('time-format-changed', {
        detail: { timeFormat: timeFormatSelect.value },
      }));
      syncRegionSelect(container);
      applyNumberLocale(readFormatState(container));
      window.myHub?.showToast(t('settings.timeFormatSavedToast'), 'success');
    } catch (error) {
      showError(errorElement, error.message);
    } finally {
...
export async function render(container, { user }) {
  try {
    ...
    safeStorageSet('my-hub-date-format', preferences.date_format);
    safeStorageSet('my-hub-time-format', preferences.time_format);
