...
import { formatDate, formatTime, t } from '/my-hub/i18n.js';
import { confirmModal } from '/my-hub/components/modal.js';
import { formatCronSchedule } from '/my-hub/settings/cron-label.js';
import {
  createDisclosure,
  createInfoRow,
  createRetryState,
  toggleRowHtml,
} from '/my-hub/settings/components.js';
...
function buildCliContent() {
  ...
  wrap.insertAdjacentHTML('beforeend', `
    ...
    <pre class="settings-code-block"><code>SERVICE=myhub
BACKUP="$PWD/my-hub-backup.db"
...
docker compose up -d "$SERVICE"</code></pre>
    ...
    <pre class="settings-code-block"><code>docker compose exec myhub node -e "import('./server/db.js').then(async db =&gt; { await db.backupToFile('/data/my-hub-backup.db'); process.exit(0); })"
docker cp myhub:/data/my-hub-backup.db ./my-hub-backup.db</code></pre>
  `);
  return wrap;
}
    ...
    if (triggerBtn) {
      triggerBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          window.myHub?.showToast(t('settings.backupSchedulerTriggeredToast'), 'success');
  ...
  if (triggerBtn) {
    triggerBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.myHub?.showToast(t('settings.backupWebdavTriggeredToast'), 'success');
...
async function loadWebdavConfig(container) {
  ...
  try {
    ...
    setVal('webdav-path', d.remotePath ?? '/myhub/backups/');
...
function bindWebdavBackupEvents(container) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      remotePath: form.querySelector('#webdav-path')?.value?.trim() || '/myhub/backups/',
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.backupWebdavSaved'), 'success');
      loadWebdavConfig(container);
    } catch (err) {
      window.myHub?.showToast(err.message ?? t('common.errorGeneric'), 'danger');
    } finally {
...
function bindRestoreEvents(container) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.backupRestoredToast'), 'success');
