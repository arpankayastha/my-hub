...
function bindEvents(container, preferences) {
  ...
  enableToggle?.addEventListener('change', async () => {
    ...
    try {
      ...
      window.myHub?.setDisabledModules?.(saved);
      window.myHub?.showToast(t('settings.rewardsSaved'), 'success');
    } catch (error) {
...
function bindDefaultPoints(container, preferences) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.rewardsDefaultPointsSaved'), 'success');
    } catch (error) {
...
async function offerRebase(from, to) {
  ...
  try {
    ...
    window.myHub?.showToast(t('settings.rewardsDefaultPointsRebased', { count: updated }), 'success');
  } catch (error) {
    window.myHub?.showToast(error.message || t('common.errorGeneric'), 'danger');
  }
}
