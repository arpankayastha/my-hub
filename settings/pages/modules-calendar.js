...
import { weekStartIndex, weekdayOrder } from '/my-hub/utils/date.js';
import { toggleRowHtml } from '/my-hub/settings/components.js';
...
function bindWeekStart(container, preferences) {
  ...
  toggle.addEventListener('click', async (event) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.weekStartSaved'), 'success');
    } catch (error) {
      current = previous;
      paint(previous); // Rollback bei Fehler
      window.myHub?.showToast(error.message || t('common.errorGeneric'), 'danger');
    }
  });
}
...
async function bindEvents(container, preferences) {
  ...
  container.querySelector('#calendar-personal-link')?.addEventListener('click', (event) => {
    if (!window.myHub?.navigate) return;
    event.preventDefault();
    window.myHub.navigate(PERSONAL_CALENDAR_PATH);
  });
  ...
  durationSelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.calendarDurationSaved'), 'success');
    } catch (error) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.holidaySaved'), 'success');
    } catch (error) {
      errorElement.textContent = error.message || t('common.errorGeneric');
      errorElement.hidden = false;
    }
  ...
  syncButton.addEventListener('click', async () => {
    ...
    if (!currentPreferenceData.holiday_country) {
      window.myHub?.showToast(t('settings.holidayCountryRequired'), 'warning');
      return;
    }
    ...
    try {
      ...
      window.myHub?.showToast(t('settings.holidaySynced'), 'success');
    } catch (error) {
      window.myHub?.showToast(error.message || t('settings.holidaySyncError'), 'danger');
    } finally {
      updateSyncState();
    }
  });
